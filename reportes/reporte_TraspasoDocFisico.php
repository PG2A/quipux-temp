...
if (isset($_GET['verrad'])){
    ...
    if ($noReferenciaTxt==''){
        ...
        $noReferenciaTxt = is_array($datosRefe) ? ($datosRefe['radi_nume_text'] ?? '') : '';//numero de referencia
    }
      if ($noReferenciaTxt=='')
        $noReferenciaTxt = '------------------';
    //remitente
    ...
    $rs = $db->query($sql);
    $fecha_envio = ($rs && !$rs->EOF) ? ($rs->fields['FECHA_ENVIO'] ?? '') : '';
    if ($fecha_envio==''){
        //envio electronico
    ...
    //creacion del pdf
    $cabecerapdf = '<html><body style="margin: 5 20 30 5;">';
    $tituloDocumento =
    '<br>
    <table width="100%">
        <tr><td align="center"><font size="5"><b>Traspaso de documentos físicos</b></font></td></tr>
    </table>
    <br />';
    $subtitulo=
    '<table width="100%" cellpadding="3">
        <tr>
            <td><b>Información del documento</b></td>
        </tr>
    </table>';
    $tablaInformacion=
    ...
    $sql="select usua_codi_dest as recibidopor
        ...
        $hist_codi = ($rsmax && !$rsmax->EOF) ? ($rsmax->fields['HIST_CODI'] ?? '') : '';
        if (trim((string)$hist_codi)=='') $hist_codi = 0;
    ...
    //echo "Final: ".$sql."<br>";
    $rs = $db->query($sql);
    $filaHist           = ($rs && !$rs->EOF) ? $rs->fields : array();
    $codRecibidoPor     = $filaHist['RECIBIDOPOR'] ?? 0;
    $fecha_entrega      = substr((string)($filaHist['HIST_FECH'] ?? ''),0,16);
    ...
    $codEnviadoPor      = $filaHist['ENVIADOPOR'] ?? 0;
    //echo $codEnviadoPor;
    ...
    $observacion        = (string)($filaHist['COMENTARIO'] ?? '');
    ...
    $estado = $datosEnvio['estado'];
     ...
     if(strlen($observacion) > 70) {  // comprobamos que el texto tiene mas de 70 caracteres
        ...
        if ($pos !== false) $observacion = substr($observacion,0,$pos);
    ...
    '<table width="100%" cellpadding="3">
        <tr>
            <td><b>Información del traspaso</b></td>
        </tr>
    </table>';
    ...
    '<table border="1" width="100%" cellpadding="3">
     ...
     //firmas
     $firmas ='<table width="100%" cellpadding="3"><tr>
    ...
    <td align="center">&nbsp;</td>
     ...
     //echo $doc_pdf;
     enviarPdf($ruta_raiz,ObtenerRutaMembrete($_SESSION["depe_codi"], 0, $_SESSION["inst_codi"], $db)["ruta"],$doc_pdf,$servidor_pdf);
}
...
$plantilla = is_file((string)$area) ? $area : "";

$pdf = ws_generar_pdf($doc_pdf, $plantilla, $servidor_pdf,"","","",90,"R");

// Cualquier aviso de PHP emitido antes de este punto corrompe el binario: se descarta.
$basura = ob_get_length() !== false ? ob_get_clean() : '';

if (!is_string($pdf) || strncmp($pdf, "%PDF", 4) !== 0) {
    header("Content-Type: text/html; charset=UTF-8");
    include_once(dirname(__DIR__).'/funciones_interfaz.php');
    echo html_error("No se pudo generar el comprobante de traspaso f&iacute;sico. El servicio de generaci&oacute;n de PDF no respondi&oacute; correctamente.");
    return;
}
