...
class GenerarDocumento {
    ...
    *******************************************************************************/
    function GenerarPDF($radi_nume, $firma_digital="no") {
        ...
        if (!$this->inicializar_variables($radi_nume, $firma_digital)) {
            if (trim((string)$this->path_pdf) === "") {
                require_once $this->ruta_raiz."/interconexion/generar_pdf.php";
                quipux_log_pdf('GenerarPDF/inicializar_variables', 'radicado=' . $radi_nume
                    . ' temp=' . ($this->registro["radi_nume_temp"] ?? 'sin_datos')
                    . ' estado=' . ($this->registro["estado"] ?? 'sin_datos')
                    . ' motivo=' . (empty($this->registro) ? 'ObtenerDatosRadicado devolvio vacio' : 'documento de entrada o sin plantilla'));
            }
...
//echo str_replace("<", "&lt;", $this->documento_html);
        ...
        $this->documento_html = '<!DOCTYPE html>
            <html>
                <head>
    ...
    *******************************************************************************/
    function inicializar_variables($radi_nume, $firma_digital) {
        ...
        if ($this->registro["arch_codi"]>0) {
            ...
            return false;
        } elseif (trim((string)($this->registro["radi_path"] ?? ''))!="") {
        ...
        if ($this->registro_padre["usar_plantilla"] == 1) {
            $membrete = ObtenerRutaMembrete($this->remitente[0]["depe_codi"], $this->registro_padre["radi_tipo"], $this->registro_padre["inst_actu"], $this->db);
            $this->plantilla_documento = $membrete["ruta"];
        }
        ...
        $this->datos_doc_frase_remitente = "<br><b>".$this->formatear_datos_documento(trim((string)($this->insitucion["fraseDespedida"] ?? '')),"U")."</b>";
    ...
    ***************************************************************************/
    function cargar_datos_destinatario () {
        ...
        $this->datos_doc_destinatario = "";
        $label_memo = "<b>PARA:</b>";
        if (!is_array($this->destinatario)) return;
...
//                    $this->datos_doc_destinatario .= "<br><b>".$this->formatear_datos_documento($usr["usua_cargo"],"I")."</b><br>&nbsp;<br>";
                ...
                default : //Oficios
                    ...
                    if (trim((string)($usr["usua_titulo"] ?? ''))!="") $this->datos_doc_destinatario .= $this->formatear_datos_documento($usr["usua_titulo"], "I")."<br>";
                    if (trim((string)(($usr["usua_nomb"] ?? '') . ($usr["usua_apellido"] ?? '')))!="") $this->datos_doc_destinatario .= $this->formatear_datos_documento($usr["usua_nomb"]." ".$usr["usua_apellido"], "I")."<br>";
                    if (trim((string)($usr["usua_cargo_cabecera"] ?? ''))!="") $this->datos_doc_destinatario .= "<b>".$this->formatear_datos_documento($usr["usua_cargo_cabecera"],"I")."</b><br>";
                    if (trim((string)($usr["usua_institucion"] ?? ''))!="") $this->datos_doc_destinatario .= "<b>".$this->formatear_datos_documento($usr["usua_institucion"],"U")."</b><br>";
                    break;
            }
        }
    ...
    ***************************************************************************/
    function cargar_datos_remitente () {
        ...
        if (!is_array($this->remitente)) return;
        for ($i=0; $i<count($this->remitente); ++$i) {
            $usr = $this->remitente[$i];
            //Quitamos las abreviaturas de Sr., Sra, etc. del titulo del remitente
            $abr_titulo = str_ireplace(array("sr.", "sra.", "srita.", "sres."), "", (string)($usr["usua_abr_titulo"] ?? ''));
            if (trim((string)($abr_titulo ?? '')) != "") $usr["usua_abr_titulo"] = $abr_titulo;
...
//        echo $sql;
        ...
        if (trim($responsables) == "")
            $responsables = strtoupper(substr(trim($this->remitente[0]["usua_nomb"] ?? ""),0,1).substr(trim($this->remitente[0]["usua_apellido"] ?? ""),0,1));
    ...
    ***************************************************************************/
    function generar_archivo_pdf() {
        ...
        if ($pdf === "0" or trim((string)$pdf) === "") {
            quipux_log_pdf('generar_archivo_pdf', 'radicado=' . ($this->registro["radi_nume_radi"] ?? '?')
                . ' temp=' . ($this->registro["radi_nume_temp"] ?? '?')
                . ' estado=' . ($this->registro["estado"] ?? '?')
                . ' formato=' . $this->formato_documento
                . ' plantilla=' . $this->plantilla_documento
                . ' plantilla_existe=' . (is_file((string)$this->plantilla_documento) ? 'si' : 'NO')
                . ' html=' . strlen((string)$this->documento_html) . 'b'
                . ' servidor=' . $servidor_pdf);
            return;
        }
        ...
        if (in_array($this->registro["estado"],array(1,3,4,7))) { // si es temporal
            ...
            if ($rs_archivo && !$rs_archivo->EOF && (0+$rs_archivo->fields["ARCH_CODI"]) != 0) {
                ...
                quipux_log_pdf('generar_archivo_pdf/bodega', 'radicado=' . $this->registro["radi_nume_radi"]
                    . ' func_grabar_archivo no devolvio arch_codi; se entrega el PDF desde bodega/tmp sin guardarlo'
                    . ' | ' . substr((string)$db_bodega->conn->ErrorMsg(), 0, 200));
                $this->path_pdf = "/tmp/".$this->registro["radi_nume_radi"].".pdf";
                file_put_contents($this->ruta_raiz."/bodega".$this->path_pdf, base64_decode($pdf));
            }
        }
    ...
    *****************************************************************************/
    function formatear_datos_documento($texto, $case="") {
        ...
        if ($case == "U") { // Pasar a mayúsculas
            $texto = str_replace($minusculas, $mayusculas, strtoupper($texto_str));
        } elseif ($case == "L") { //Minúsculas
            $texto = str_replace($mayusculas, $minusculas, strtolower($texto_str));
        } elseif ($case == "I") { //Intercalar
            // Por versiones anteriores en las que todo el texto podía estar todo en mayúsculas o minúsculas
            if ($texto_str == strtoupper($texto_str) or $texto_str == strtolower($texto_str)) {
                $texto = str_replace($mayusculas, $minusculas, strtolower($texto_str));
                $cadena = "";
