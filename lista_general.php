...
if (isset ($replicacion) && $replicacion && $config_db_replica_info_lista_general!="") {
    $db = new ConnectionHandler(__DIR__, $config_db_replica_info_lista_general);
}
...
<table border="0" width="100%" align="left" class="borde_tab" id=tb_general>
    ...
    <tr>
    	...
    	<td width="25%" class="listado2"><?=substr((string)($datosrad["radi_fecha"] ?? ''),0,10).($descZonaHoraria ?? '') ?></td>
    	<td width="20%" class="titulos2" align="right">Tipo de Documento:&nbsp;&nbsp;</td>
    	<td width="35%" class="listado2">
            <span id="span_tipo_doc"></span>&nbsp;
            <img src="/imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("tipo_doc",2);' id="img_tipo_doc" align="middle" border="0" title="Modifica el tipo de documento" alt="editar">
<?php
...
?>
    ...
    <tr>
        <?php if(trim((string)($datosrad["radi_referencia"] ?? '')) == "" ) $numCol = 3; else $numCol = 1;?>
	<td class="titulos2" align="right">Asunto:&nbsp;&nbsp; </td>
	<td class='listado2' colspan="<?=$numCol?>"><?=$datosrad["radi_asunto"] ?> </td>
        <?php if(trim((string)($datosrad["radi_referencia"] ?? '')) != "" ) { ?>
        <td class="titulos2" align="right"><?=$descReferencia ?? 'Referencia'?>:&nbsp;&nbsp;</td>
           ...
           if ($nivel_seguridad_documento >= 3) {
               ...
               if (trim((string)($datosrad["radi_padre"] ?? ''))!=''){//si tiene
                    $radRefe=ObtenerDatosRadicado($datosrad["radi_padre"],$db);
                    $mostrar=1;
               } elseif (trim((string)($datosrad["radi_nume_asoc"] ?? ''))!="") {//si no tiene tomo la asociacion
                ...
                if (isset ($radRefe)) {
                    ...
                    if (file_exists(__DIR__."/bodega".$radRefe["radi_path"]) or $radRefe["arch_codi"]>0 or $radRefe["radi_imagen"]!=''){//si existe eel archivo
                        ...
                        if ($nivel_seguridad_refe >= 2)
                            $graficapath= "&nbsp;<img src='../imagenes/zoom_in.png' width='15' height='15' alt='Vista Previa' border='0'
    ...
    <tr>
<?php   // Mostrar links de descarga del documento
        ...
        if ($nivel_seguridad_documento >= 2) {
            ...
            if (substr((string)($datosrad["radi_nume_temp"] ?? ''),-1)=="2" && trim((string)($datosrad["radi_path"] ?? ''))=="" && trim((string)($datosrad["radi_imagen"] ?? ''))=="")
                $imagenv = "<span class='vinculos'>Documento Digitalizado no disponible.</span>";

            if (trim((string)($datosrad["fecha_firma"] ?? ''))!="" or trim((string)($datosrad["usua_firma"] ?? ''))!="")
...
<?php if (trim((string)($datosrad["usua_firma"] ?? ''))!="") { ?>
    <tr>
...
?>
        <tr <?php if ($mostrarDirigido==0) echo 'style="display: none"';?>>
            ...
            <td class='listado2' colspan="3">
                ...
                <img src="../imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("usua_redirigido",2);' id="img_usua_redirigido" align="middle" border="0" title="Modifica el usuario a quien sera dirigido el documento" alt="editar">
<?php
...
?>
    ...
    <tr>
        ...
        <td class='listado2' colspan="1">
            ...
            <img src="../imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("nivel_seguridad",2);' id="img_nivel_seguridad" align="middle" border="0" title="Modifica el nivel de seguridad del documento" alt="editar">
            <select name="txt_nivel_seguridad" class="select"  id="txt_nivel_seguridad" style="display:none;" onchange='modificar_opcion("nivel_seguridad")'>
                <option value="0" <?php if($datosrad["seguridad"]==0) echo "selected"?>>P&uacute;blico</option>
                <option value="1" <?php if($datosrad["seguridad"]==1) echo "selected"?>>Confidencial</option>
            </select>
<?php
...
?>
	<td class="titulos2" align="right"> <?=$descTRDpl ?? 'TRD'?>:&nbsp;&nbsp; </td>
	<td class="listado2" colspan="1">
<?php
            echo MostrarTRD($verrad, $usr_actual["depe_codi"], $db);
            if($nivel_seguridad_documento > 3)
                echo "&nbsp;&nbsp;<img src='../imagenes/internas/pencil_add.png' id='img_trd' border='0' title='Incluir el documento en una $descTRD' alt='editar' onClick='CambiarTRD();'>";
?>
	</td>
    </tr>
    <tr>
        ...
        <td class="listado2">
            ...
            <img src="../imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("categoria",2);' id="img_categoria" align="middle" border="0" title="Modifica la categor&iacute;a del documento" alt="editar">
<?php
...
?>
        ...
        <td class="listado2">
            ...
            <img src="../imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("tipificacion",2);' id="img_tipificacion" align="middle" border="0" title="Modifica la tipificaci&oacute;n del documento" alt="editar">
<?php
...
<?php
    ...
    if (trim((string)($datosrad["radi_resumen"] ?? ''))!="" or $nivel_seguridad_documento>=6 or ($datosrad["estado"]==6 and $datosrad["usua_actu"]==$_SESSION["usua_codi"])) {
?>
    <tr>
        <td class="titulos2" align="right"> Notas:&nbsp;&nbsp;</td>
        <td class="listado2" colspan="3" valign="top">
            <span id="span_radi_resumen"></span>&nbsp;
            <img src="../imagenes/internas/pencil_add.png" onclick='modificar_opcion_mostrar("radi_resumen",2);' id="img_radi_resumen" align="middle" border="0" title="Modifica el resumen del documento" alt="editar">
            <textarea name="txt_radi_resumen" id="txt_radi_resumen" cols="100" class="tex_area" rows="3" onchange='this.value=this.value.substring(0,950)'><?=trim((string)($datosrad["radi_resumen"] ?? ''))?></textarea>
            &nbsp;&nbsp;
            <img src="../imagenes/disk.png" onclick='modificar_opcion("radi_resumen");' id="img_guardar_radi_resumen" style="vertical-align: top" border="0" title="Graba el resumen del documento" alt="editar">
<?php
...
<?php
    ...
    if (substr((string)($datosrad["radi_nume_temp"] ?? ''),-1)=='2') {
    ...
    <?php  }
    if(trim((string)($datosrad["radi_desc_anexos"] ?? '')) != '') { ?>
    <tr>
	...
	<td class="listado2" colspan="3">
        <?php
            ...
            if($nivel_seguridad_documento > 3)
                echo "&nbsp;&nbsp;<img src='../imagenes/internas/pencil_add.png' id='img_trd' border='0' title='Agregar Metadatos' alt='editar' onClick='DefinirMetadato();'>";
        ?>
    </tr>
</table>
...
{
    $cadena = "";
    if ($estado==1 or $estado==7 or $estado==8 or !lista_general_tiene_depara($radicado, $tipo, $db)) {//borradores, eliminados o sin filas en usuarios_radicado
        foreach (explode('-',(string)($usuario ?? '')) as $usua_codi) {
	    if (trim((string)$usua_codi)!="") {
                ...
                if($tipo==2) { //$tipo="Para"  y $usr["tipo_usuario"]="func." y $tipodoc=oficio
                    ...
                    if (trim((string)($opc_impr["CARGO_CABECERA"] ?? '')) != "")
                        $cargo = $opc_impr["CARGO_CABECERA"];
                    else if (trim((string)($usr["cargo_cabecera"] ?? ''))!="")
...
function lista_general_tiene_depara($radicado, $tipo, $db)
{
    $rs = $db->conn->query("select 1 from usuarios_radicado where radi_nume_radi=$radicado and radi_usua_tipo=$tipo limit 1");
    return $rs && !$rs->EOF;
}
