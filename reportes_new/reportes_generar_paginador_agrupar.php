...
while (!$rs_paginador->EOF and ($num_filas < $num_max_registros or (0+$num_max_registros)==0)) {
    if ($ocultar_valor[1] != $rs_paginador->fields[strtoupper($cols[1])]) {
        ++$num_filas;
    }
    $tabla .= "<tr " . ${"estilo_tr".($num_filas % 2)} . ">";
    for ($i=1 ; $i<count($cols) ; ++$i) {
        $dato = $rs_paginador->fields[strtoupper($cols[$i])];
        if ($ocultar_valor[$i] == $dato) {
            $dato = "";
        } else {
            for ($j=$i ; $j<count($cols) ; ++$j) {
                $ocultar_valor[$j] = "";
            }
            $ocultar_valor[$i] = $dato;
        }
        if (isset($rs_paginador->fields[strtoupper($cols[$i])."_DRILL"])) {
            $tabla .= "<td><font size='1'><a href='javascript:".$rs_paginador->fields[strtoupper($cols[$i])."_DRILL"]."' $estilo_a>$dato</a>&nbsp;</font></td>";
        } else {
