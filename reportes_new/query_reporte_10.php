...
switch($db->driver) {
    case 'postgres':
        ...
        $where_inst_destino = " and radi_usua_actu in (select usua_codi from usuarios where inst_codi=".$_SESSION["inst_codi"].")";
        if (isset($txt_depe_codi) && $txt_depe_codi != "0")
            $where_inst_destino = " and radi_usua_actu in (select usua_codi from usuarios where depe_codi in ($txt_depe_codi))";
        if (isset($txt_usua_codi) && $txt_usua_codi != "0")
            $where_inst_destino = " and radi_usua_actu in ($txt_usua_codi)";

        $where_inst_origen = " and radi_usua_radi not in (select usua_codi from usuarios where inst_codi=".$_SESSION["inst_codi"].")";
        if (isset($txt_inst_codi) && $txt_inst_codi != "0")
            $where_inst_origen = " and radi_usua_radi in (select usua_codi from usuarios where inst_codi=$txt_inst_codi)";
