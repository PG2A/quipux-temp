    ...
    case 'postgres':
        ...
        $main_sql = "select h1.radi_nume_radi
                    ...
                    from (
                        ...
                        from hist_eventos 
                        where radi_nume_radi in (select radi_nume_radi from radicado where radi_inst_actu=".$_SESSION["inst_codi"]." and esta_codi in (2,0) $where_fecha)
                        and usua_codi_dest in (select usua_codi from usuarios where inst_codi=".$_SESSION["inst_codi"]." $where_usr)
                        and sgd_ttr_codigo in (2,9,28,25)
                    ...
                    left outer join hist_eventos h2 on h1.radi_nume_radi=h2.radi_nume_radi and h1.hist_codi<h2.hist_codi 
                        and ((h1.usua_codi_dest=h2.usua_codi_ori and sgd_ttr_codigo in (9,13,67))
                            or (h1.usua_codi_ori=h2.usua_codi_ori and sgd_ttr_codigo in (28)))
        ...
        $sql["from"]  .= " left outer join tiporad tr on r.radi_tipo=tr.trad_codigo ";
        $sql["where"]  = "  ";
        $sql["order"]  = " order by ";
        $sql["limit"]  = "";
        ...
        for ($i=1 ; $i<count($cols) ; ++$i) {
            ...
            switch ($cols[$i]) {
                ...
                case "hist_dest_fecha" :
                    $sql["select"] .= "substr(hd.hist_fech::text,1,19) $nomb_as, ";
                ...
                case "hist_tiempo" :
                    ...
                    break;
                case "tipo_doc" :
                    $sql["select"] .= "tr.trad_descr $nomb_as, ";
                    break;
                case "num_ref" :
                    $sql["select"] .= "r.radi_cuentai $nomb_as, ";
                    break;
                 case "firma" :
                    $sql["select"] .= "CASE WHEN r.radi_fech_firma is not null THEN 'SI' ELSE 'NO' END  $nomb_as, ";
                    break;
                default:
                    $sql["select"] .= "'' $nomb_as, ";
                    break;
