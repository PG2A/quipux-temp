...
*/
 function ConsultarCarpetaVirtual($db, $depe_actu, $es_consulta)
 {
    ...
    //Se arma el listado
    while (!$rs->EOF) {
        $lista[$i]["codigo"] = isset($rs->fields["TRD_CODI"]) ? $rs->fields["TRD_CODI"] : $rs->fields["trd_codi"];
        $lista[$i]["codigo_padre"] = isset($rs->fields["TRD_PADRE"]) ? $rs->fields["TRD_PADRE"] : $rs->fields["trd_padre"];
        $lista[$i]["nombre"] = isset($rs->fields["TRD_NOMBRE"]) ? $rs->fields["TRD_NOMBRE"] : $rs->fields["trd_nombre"];
        $lista[$i]["nivel"] = isset($rs->fields["TRD_NIVEL"]) ? $rs->fields["TRD_NIVEL"] : $rs->fields["trd_nivel"];
        
        $trd_codi_doc = isset($rs->fields["TRD_CODI_DOC"]) ? $rs->fields["TRD_CODI_DOC"] : (isset($rs->fields["trd_codi_doc"]) ? $rs->fields["trd_codi_doc"] : "");
        $trd_padre_cv = isset($rs->fields["TRD_PADRE_CV"]) ? $rs->fields["TRD_PADRE_CV"] : (isset($rs->fields["trd_padre_cv"]) ? $rs->fields["trd_padre_cv"] : "");
        
        if($trd_codi_doc != "") $tiene_doc = 1; else $tiene_doc = 0;
        if($trd_padre_cv != "") $tiene_carpeta = 1; else $tiene_carpeta = 0;
        ...
        $trd_estado = isset($rs->fields["TRD_ESTADO"]) ? $rs->fields["TRD_ESTADO"] : $rs->fields["trd_estado"];
        
        if ($trd_estado==1)
                $estado="Activo";
        else if($trd_estado==0)
            $estado="Inactivo";
        else if($trd_estado==2)
            $estado="Eliminado";
        $lista[$i]["estado"] = $estado;
        $lista[$i]["estado_valor"] = $trd_estado;
        
        $lista[$i]["arch_gestion"] = isset($rs->fields["TRD_ARCH_GESTION"]) ? $rs->fields["TRD_ARCH_GESTION"] : (isset($rs->fields["trd_arch_gestion"]) ? $rs->fields["trd_arch_gestion"] : "");
        $lista[$i]["arch_central"] = isset($rs->fields["TRD_ARCH_CENTRAL"]) ? $rs->fields["TRD_ARCH_CENTRAL"] : (isset($rs->fields["trd_arch_central"]) ? $rs->fields["trd_arch_central"] : "");
        $lista[$i]["cant_doc"] = isset($rs->fields["TRD_CANT_DOC"]) ? $rs->fields["TRD_CANT_DOC"] : (isset($rs->fields["trd_cant_doc"]) ? $rs->fields["trd_cant_doc"] : "");        
	$rs->MoveNext();
        $i++;
    }
    return $lista;
...
//			<img src='../imagenes/add.gif' border='0' alt='' height='15px' width='15px'></a>";
//	$resp .= "</td>";
