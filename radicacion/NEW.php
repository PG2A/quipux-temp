...
$ent = isset($_GET['ent']) ? limpiar_numero($_GET['ent']) : 0;
$accion = isset($_GET['accion']) ? limpiar_sql($_GET['accion']) : '';
$krd = $_SESSION['krd'];
...
$carpeta = isset($_GET['carpeta']) ? limpiar_numero($_GET['carpeta']) : 0;
$documento_us1 = '';
$documento_us2 = '';
$concopiaa = '';
$radi_lista_dest = '';
...
$txt_refeResponder = '';
$lista_modificada = 0;
$fecha_doc = date("Y-m-d"); // Default to current date
$desc_anex = ''; // Initialize description of attachments

// Flags que deciden si se sugiere Oficio o Memorando; los recalcula
// radicacion/buscar_usuario_de_para.php cada vez que cambian los destinatarios.
// Mientras no haya destinatarios no se debe sugerir ningun cambio de tipo, asi que
// el valor neutro es flag_inst=0 (no son todos externos) y flag_inst_m=1 (no son
// todos internos). Sin estas inicializaciones se imprimian como value="" y en
// JavaScript "" == 0 es verdadero, disparando la sugerencia de Memorando en un
// documento nuevo sin ningun destinatario seleccionado.
$flag_inst = 0;
$flag_inst_m = 1;
...
<script type="text/javascript" src="../js/crea_combos_2.js"></script>
...
<?php
    $ruta_raiz = "..";
    $noRad = $nurad ?? '';
    // Ensure textrad is safe for JS string
    $textrad_js = $textrad ?? '';
?>
<script type="text/javascript">
 var objetoImpresion=new Array();
    ...
    function vista_previa() {
        ...
        url = "<?=$ruta_raiz?>/VistaPrevia.php?verrad=<?=$nurad?>&archivo=&textrad=<?=$textrad_js?>";
...
// Devuelve el flag como numero. Si el hidden esta vacio o no es numerico usa
// por_defecto, que debe ser el valor neutro (el que no sugiere cambiar de tipo).
function normalizar_flag_inst(valor, por_defecto) {
    var n = parseInt(valor, 10);
    return isNaN(n) ? por_defecto : n;
}
...
function refrescar_pagina(origen,cod_impresion)
{
    ...
    // Si el hidden llega vacio (el popup no alcanzo a copiarlo) hay que caer al valor
    // neutro, no al 0: en JavaScript "" == 0 es verdadero y sugeriria Memorando sin
    // que haya destinatarios.
    flag_inst = normalizar_flag_inst(document.formulario.flag_inst.value, 0);
    flag_inst_m = normalizar_flag_inst(document.formulario.flag_inst_m.value, 1);
    ...
    if (document.formulario.hidden_actualiza_opciones.value==1){
        ...
        nuevoAjax('div_borrar_opc_imp', 'GET', '/tx/tx_borrar_opcion_imp.php', datos); 
        pestanas(1);
    }       
...
//                    else return false;
    ...
    function cuerpo_editado() {
        try {
            var oEditor = CKEDITOR.instances.raditexto;
            if (oEditor && oEditor.checkDirty) return oEditor.checkDirty();
        } catch (e) {}
        return false;
    }
...
//        timerID = setTimeout("mostrar_botones (true)", 5000);
    ...
    function cambio_cuerpo_editor() {
        try {
            ...
            // SECURITY INTERCEPTOR: Detect if the payload was pre-empted by a Quipux Custom Error Handler or standard PHP Warning
            var trimmedTexto = texto.trim();
            if (trimmedTexto.substring(0, 4) === '<div' || trimmedTexto.substring(0, 2) === '<b') {
                document.getElementById('raditexto').value = trimmedTexto;
                var oEditor = CKEDITOR.instances.raditexto;
                oEditor.setData('<div style="margin:20px; padding:10px; border:2px dashed red; background:#ffeeee;"><strong>SYSTEM ERROR CAUGHT:</strong><br><br>' + trimmedTexto + '</div>');
                return;
            }
            ...
            var oEditor = CKEDITOR.instances.raditexto;
            oEditor.setData(texto, function() {
                this.resetDirty();
                cuerpo_documento = trim(this.getData());
            });
        } catch (e) {}
    }
...
{
        <?php
        $carpeta_regreso = (isset($carpeta) && $carpeta !== '') ? $carpeta : 1;
        if ($nurad && trim($accion)=="Editar")
            $var_envio = "window.location='$ruta_raiz/verradicado.php?verrad=$nurad&menu_ver=3&irVerRad=1&tipo_ventana=popup';";
        else
            $var_envio = "window.location='$ruta_raiz/cuerpo.php?carpeta=$carpeta_regreso&adodb_next_page=1';";
        echo $var_envio;
        ?>
    return;
}
function validarDoc(){
    resp = 1;
    flag_inst = normalizar_flag_inst(document.getElementById("flag_inst").value, 0);
...
var quipux_grabando = false;
function grabar_doc(dato)
{
       if (dato == 2 && quipux_grabando) return;
       if (validarDoc()==1){
            if (<?=$ent?>==2) {
            ...
            if (trim(document.formulario.documento_us1.value) != '' && trim(document.formulario.documento_us2.value) != '' ) {
                if (trim(document.formulario.asunto.value) != '') {
                    try {
                        if (typeof CKEDITOR != "undefined" && CKEDITOR.instances.raditexto) {
                            document.getElementById('raditexto').value = CKEDITOR.instances.raditexto.getData();
                        }
                    } catch(e) {}
                    ...
                    if (dato == 2 && document.getElementById('hidd_ent') && document.getElementById('hidd_ent').value == '2') {
                        ...
                        if (document.getElementsByName('Submit2')[0]) document.getElementsByName('Submit2')[0].disabled = true;
                        if (window.bloquearPantalla) bloquearPantalla('Guardando documento...');
                    }
...
//            }
	...
	var y = (screen.height - 575) / 2;
        ...
        url = "" + "/Administracion/ciudadanos/adm_usuario_ext.php?cerrar=Si&accion=2&ciu_codigo=" + document.getElementById("usuaCodi").value+"&cod_impresion="+cod_impresion;
...
//        }
    ...
    if(trim($accion)=="Editar" || trim($accion)=="Responder" || trim($accion)=="ResponderTodos" || trim($accion)=="Copiar")//editar responder
    {
        $radicado=ObtenerDatosRadicado($nurad,$db);
        if (empty($radicado)) {
            die(html_error("No se encontr&oacute; informaci&oacute;n del documento ($nurad)."));
        }

        // Al responder desde la bandeja la URL solo trae nurad; textrad se pierde y con
        // el se pierden el campo "No. Referencia" y el enlace al documento padre (ver
        // $referencia / $nurad_referencia mas abajo). El dato correcto esta en el
        // radicado que acabamos de cargar.
        if (trim((string)$textrad) === '') $textrad = $radicado["radi_nume_text"] ?? '';
        ...
        $cod_codi = 0 + ($radicado["cod_codi"] ?? 0);
        $cat_codi = 0 + ($radicado["cat_codi"] ?? 0);
        ...
        $radi_lista_nombre_get = $_GET['radi_lista_nombre'] ?? '';
        if ($radi_lista_nombre_get == '')
        ...
        $txt_usua_redirigido = $radicado["usua_redirigido"] ?? 0;
        $chk_ocultar_recorrido = $radicado["ocultar_recorrido"] ?? 0;
        ...
        elseif($accion=="ResponderTodos"){
            ...
            $fecha_doc	= substr((string)$fecha_doc,8 ,2)."-".substr((string)$fecha_doc,5 ,2)."-".substr((string)$fecha_doc,0 ,4);
            ...
            if (trim($accion)!="Copiar" && ($radicado["estado"] != 1 or $_SESSION["usua_codi"] != $radicado["usua_actu"])) {
                die (html_error("Usted no tiene los permisos suficientes para modificar este documento."));
            }
            if (trim($accion)=="Copiar") {
                $es_remitente = false;
                foreach (explode("--", trim((string)($radicado["usua_rem"] ?? ''), "-")) as $id_rem) {
                    if (trim($id_rem) !== '' && (int)$id_rem == (int)$_SESSION["usua_codi"]) { $es_remitente = true; break; }
                }
                $es_elaborador = ((int)$radicado["estado"] == 1 && (int)$_SESSION["usua_codi"] == (int)$radicado["usua_actu"]);
                if (!$es_remitente && !$es_elaborador) {
                    die (html_error("Solo el remitente del documento puede copiarlo."));
                }
            }
    	}
    ...
    if(trim($accion)=="Copiar")
    {
        ...
        $radi_padre_copia = $refe_padre ?? ''; // Use safe variable
...
<body bgcolor="#FFFFFF" onLoad="pestanas(1); cambiar(); mostrarOpcImp(); cargar_editor(); seleccionar_anexos_copiar_anexos()">
    <?php dibujar_loader_pantalla('Guardando documento...', '/imagenes/escudo_blanco.png'); ?>
...
<tr>
    ...
    <tr valign="top">
        ...
        <?php $flag_inst = $flag_inst ?? ''; $flag_inst_m = $flag_inst_m ?? ''; ?>
        ...
        <tr valign="middle">
            ...
            ?>
                ...
                <?php
                    ...
                    if(!$rsCat->EOF)
                        print $rsCat->GetMenu2("cat_codi", (int)$cat_codi,  "", false,"","class='select' " );
                ?>
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <?php
                    $queryCod = "Select cod_descripcion, cod_codi from codificacion where inst_codi in (0,".((int)$_SESSION["inst_codi"]).") order by 1";
                    ...
                    if(!$rsCod->EOF)
                    {
                        ...
                        <?php
                        print $rsCod->GetMenu2("cod_codi", (int)$cod_codi,  "", false,"","class='select' style='width:220px' " );
                    }
                ?>
                </td>
                </tr>
<?php  } //IF CIUDADANO  ?>
                ...
                value="<?= htmlspecialchars((string)$referencia, ENT_QUOTES) ?>" <?=$readReferencia?> onkeyup="validaEspacio(event, this, '<?=$readReferencia?>');" onblur="quitarCaracter(this, ' ');"/>

                <?php
                // MOSTRAR ICONOS DE REFERENCIA
                $nurad_referencia = '';
                if (isset($radicado["radi_padre"]) && $radicado["radi_padre"]!='') {
                ...
                if ($nurad_referencia != '') {
                    ...
                    //echo "seguridad: ".$nsd;
                    if (isset($radRefe["radi_path"]) && (file_exists(dirname(__DIR__)."/bodega".$radRefe["radi_path"]) or $radRefe["arch_codi"]>0 or $radRefe["radi_imagen"]!='')){//si existe eel archivo
                        $ventana = dirname(__DIR__)."/documento_online.php?verrad=$nurad_referencia";
                        $nivel_seguridad_refe = obtener_nivel_seguridad_documento($db, $nurad_referencia);
                        if ($nivel_seguridad_refe >= 2) {
                            echo "&nbsp;<img src='../imagenes/zoom_in.png' width='17' height='17' alt='Vista Previa' border='0'
                                        title='Ver en l&iacute;nea Documento de Referencia' onClick='ventanaNueva(\"$ventana\");'>
                                    &nbsp;<img src='../imagenes/document_down.jpg' width='17' height='17' alt='Vista Previa' border='0'
                ...
                <tr>
                    ...
                    <td class="listado1" colspan="4">
                        <textarea id="asunto" name="asunto" cols="150" class="tex_area" rows="1" onkeypress="return limita(event,'asunto','spn_numero_caracteres_disponibles',250);" ><?php echo htmlspecialchars((string)$asunto, ENT_QUOTES) ?></textarea>
                        ...
                        <input id="notas" name="notas" type="hidden" value="<?php echo htmlspecialchars((string)$notas, ENT_QUOTES) ?>">
                    </td>
                </tr>
            ...
            ?>
                                ...
                                <td width="35%" align="left" class="listados1" valign="middle">&nbsp;
            <?php
            if (isset($_SESSION["perm_borrar_recorrido"]) && $_SESSION["perm_borrar_recorrido"] == 1) {
                echo '<input type="checkbox" name="chk_ocultar_recorrido" id="chk_ocultar_recorrido" value="1"';
                if (isset($chk_ocultar_recorrido) && $chk_ocultar_recorrido == "1") echo ' checked';
        ...
        <?php 
        $radi_tipo_impresion = $radi_tipo_impresion ?? '';
        if ($ent!=2) {?>
                    ...
                    <?php
                        if ($ent==1) {
                            if ($chk_plantilla==1)
                                ...
                                $con_plantilla="0";
                            $checked_attr = ($con_plantilla == 'checked') ? 'checked="checked"' : '';
                            ?>
                            <input type="checkbox" name="chk_plantilla" id="chk_plantilla" value="1" <?php echo $checked_attr; ?> onclick="histop('l',this,'<?php echo $con_plantilla; ?>');" /> Utilizar Plantilla
                        <?php }
                    ?>
                </td>
            </tr>
    ...
    <?php
        ...
        <input type="hidden" id="codiOpcImp" name="codiOpcImp" value="<?= htmlspecialchars((string)((isset($rsOpcImprBusq) && is_array($rsOpcImprBusq) && isset($rsOpcImprBusq['OPC_IMP_CODI'])) ? $rsOpcImprBusq['OPC_IMP_CODI'] : ''), ENT_QUOTES) ?>"/>
...
<?php
        ...
        $verrad = $nurad;
        if ($nurad != "") {
            include dirname(__DIR__)."/anexos/anexos.php";
        } else {
