...
if (isset ($replicacion) && $replicacion && $config_db_replica_buscar_usuario_nuevo!="") {
    $db = new ConnectionHandler(dirname(__DIR__), $config_db_replica_buscar_usuario_nuevo);
}
...
function pasar_datos() {
    ...
    if(document.formu1.documento_us1.value != '' &&  document.formu1.documento_us2.value != '')
    {
        ...
        if (opener.document.formulario.documento_us1) opener.document.formulario.documento_us1.value = document.formu1.documento_us1.value;
        if (opener.document.formulario.documento_us2) opener.document.formulario.documento_us2.value = document.formu1.documento_us2.value;
        if (opener.document.formulario.concopiaa) opener.document.formulario.concopiaa.value = document.formu1.concopiaa.value;
        if (opener.document.formulario.fl_modificar1) opener.document.formulario.fl_modificar1.value = "1";
        //actualizar opciones de impresion
        if (opener.document.formulario.num_rad) {
            ...
            if (radicadonum!=''){
                if (destinatario_anterior!=opener.document.formulario.documento_us1.value){
                    if (opener.document.formulario.hidden_actualiza_opciones) opener.document.formulario.hidden_actualiza_opciones.value = 1;
                }
            }
        }
        /**
        ...
        {
            if (opener.document.formulario.radi_tipo_impresion) opener.document.formulario.radi_tipo_impresion.value = document.formu1.tipo_impresion.value;
            if (opener.document.formulario.radi_lista_dest) opener.document.formulario.radi_lista_dest.value = "-" + document.formu1.lista_usr.value + "-";
        }*/

        /**
        * Pasar nombre de listas seleccionadas.
        **/
        if (opener.document.formulario.radi_lista_dest) opener.document.formulario.radi_lista_dest.value=document.formu1.radi_lista_dest.value;
        if (opener.document.formulario.radi_lista_nombre) opener.document.formulario.radi_lista_nombre.value=document.formu1.radi_lista_nombre.value;
        /**
        * Pasar id de instituciones.
        **/
        //Para determinar que tipo de documento se tiene que generar Memo u Oficio
        if (opener.document.formulario.flag_inst && document.getElementById('flag_inst')) opener.document.formulario.flag_inst.value=document.getElementById('flag_inst').value;
        if (opener.document.formulario.flag_inst_m && document.getElementById('flag_inst_m')) opener.document.formulario.flag_inst_m.value=document.getElementById('flag_inst_m').value;
...
function pasar(cod_usr, tipo, flag_lista)
{
    ...
    if(tipo==2 && band_usr2) {
        document.formu1.documento_us2.value <?php if ($ent==2) echo "+";?>= '-' + cod_usr + '-';
    }
    if(tipo==3 && band_cca) {
...
<?php  
$lista_modificada = isset($_GET['listado_listas']) ? (int)($_GET['lista_modificada'] ?? 0) : 0;
echo '<input type="hidden" name="hidden_lista_modificada" id="hidden_lista_modificada" class="tex_area" value="'.$lista_modificada.'"/>';
?>
<table border=0 width="100%" class="borde_tab popup-buscar" cellpadding="0" cellspacing="5">
  <tr>
...
<table class=borde_tab width="100%" cellpadding="0" cellspacing="4">
    <tr class=listado2>
        <td colspan="10">
            <center><b>DATOS A COLOCAR EN EL <?=strtoupper((string)($descRadicado ?? ''))?></b></center>
        </td>
    </tr>
    <tr>
