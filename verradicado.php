...
require_once(__DIR__.'/tipo_documental/obtener_datos_trd.php');
include_once(__DIR__.'/seguridad/obtener_nivel_seguridad.php');
include_once(__DIR__.'/obtenerdatos.php');
include_once(__DIR__.'/metadatos/metadatos_funciones.php');
include_once(__DIR__.'/funciones_interfaz.php');

$verrad = $_GET['verrad'] ?? $_POST['verrad'] ?? '';
$carpeta = $_GET['carpeta'] ?? $_POST['carpeta'] ?? '';
$estadisticas = $_GET['estadisticas'] ?? $_POST['estadisticas'] ?? 0;
$menu_ver = $_GET['menu_ver'] ?? $_POST['menu_ver'] ?? '';
$tipo_ventana = $_GET['tipo_ventana'] ?? $_POST['tipo_ventana'] ?? '';
...
<script type="text/javascript">
    function fjs_radicado_descargar_archivo(radicado, anex_codigo, arch_tipo, tipo_descarga) {
        ...
        if (tipo_descarga=='embeded') {
            if (document.getElementById('ifr_mostrar_archivo')) {
                document.getElementById('ifr_mostrar_archivo').src=path_descarga;
                document.getElementById('ifr_mostrar_archivo').style.display='';
            }
        } else {
            if (document.getElementById('ifr_descargar_archivo')) document.getElementById('ifr_descargar_archivo').src=path_descarga;
        }
        return;
    }
...
<body bgcolor="#FFFFFF" onLoad="fjs_popup_crear_divs(); window_onload(<?=$radi_tipo?>); <?=$path_archivo_embebido?>">
    <form name="form1" id="form1" action="./tx/formEnvio.php?carpeta=<?=$carpeta?>" method="post">

<?php
        $dirresponder = "./radicacion/NEW.php?nurad=$verrad&radi_padre=$verrad&textrad=$textrad&carpeta=$carpeta&accion=Responder";
        $dirresponderTodos = "./radicacion/NEW.php?nurad=$verrad&radi_padre=$verrad&textrad=$textrad&carpeta=$carpeta&accion=ResponderTodos";
        $dirmodificar = "./radicacion/NEW.php?nurad=$verrad&textrad=$textrad&carpeta=$carpeta&accion=Editar";
        $dircopiar = "./radicacion/NEW.php?nurad=$verrad&radi_padre=$verrad&textrad=$textrad&carpeta=$carpeta&accion=Copiar";
        $controlAgenda=1;
        if ($estadisticas==1 or ($_SESSION["usua_tipo"] ?? 0)==2) $estado=100;
...
<?php $descDependencia = $_SESSION['descDependencia'] ?? 'Área'; ?>
<div class="verrad-banner">
<table width="100%" border="0" cellpadding="0" cellspacing="1" >
    <tr>
        <td class="titulos4" width="40%"><span class="vb-label">No. Documento:</span> <span class="vb-num"><?=$datosrad["radi_nume_text"] ?></span></td>
        <td class="titulos4" align="left" width="35%"><span class="vb-label">Usuario actual:</span> <span class="vb-val"><?=$usr_actual["nombre"]?></span></td>
        <td class="titulos4" align="left" width="25%"><span class="vb-label"><?=$descDependencia?> actual:</span> <span class="vb-val"><?=$usr_actual["dependencia"]?></span></td>
    </tr>
</table>
</div>

<div onclick="desplegarContraer('documento',this);" class="linkExpandido verrad-heading">Datos del Documento</div>
...
if((($_SESSION["cargo_tipo"] ?? '')=='1' or ($_SESSION["usua_perm_mostrar_documento"] ?? 0)==1) and $nivel_seguridad_documento>=2 and $menu_ver==3)
{
    echo "<iframe name='ifr_mostrar_archivo' id='ifr_mostrar_archivo' class='visor-documento' style='display:none'
            src=''>
            Su navegador no soporta iframes, por favor actualicelo.</iframe>";
}
?>
