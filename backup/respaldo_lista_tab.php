 ...
 */
  ...
  $txt_tipo_lista = isset($_GET["txt_tipo_lista"]) ? trim(limpiar_numero($_GET["txt_tipo_lista"])) : 1; //1: Muestra listado del usuario actual, 2: Muestra listado por autorizar
  ...
  $cmb_estado = trim(limpiar_sql($_GET["cmb_estado"] ?? 0));
  $txt_sol = trim(limpiar_numero($_GET["txt_sol"] ?? ''));
  $cmb_institucion = trim(limpiar_numero($_GET["cmb_institucion"] ?? 0));
  $usr_cedula = $usr_cedula ?? "";
  $paginador = new ADODB_Pager_Ajax(dirname(__DIR__), "div_buscar_solicitudes", "respaldo_buscar.php", "cmb_estado,txt_nombre,txt_fecha_inicio_sol,txt_fecha_fin_sol,txt_sol,txt_tipo_lista,cmb_institucion","");
...
<script language="JavaScript" type="text/JavaScript">
    function buscar_solicitudes() {
      ...
      {       
       ...
       //paginador_reload_div('');
        nuevoAjax('div_buscar_solicitudes', 'GET', '/backup/respaldo_buscar_tab.php', parametros, '');
    ...
    function seleccionar_solicitud(codigo, lista) {
        ...
        if(tipo_lista == 1 || tipo_lista == 2 || tipo_lista == 6 || tipo_lista == 9 || tipo_lista == 11){
            var_envio='/backup/respaldo_informacion.php?txt_resp_soli_codi='+codigo+'&txt_tipo_lista='+lista;
            window.location=var_envio;
        }
        else{
            var_envio='/backup/respaldo_informacion.php?txt_resp_soli_codi='+codigo+'&tipo_ventana=popup&txt_tipo_lista='+lista;
  ...
  function descargar_respaldo(codigo, solicitud) {
        ...
        url = '/backup/backup_usuarios_descargar_archivos.php?codigo_backup=' + codigo +'&resp_soli_codi='+solicitud;
    ...
    <form name="formulario" action="" method="post">
        ...
        <table width="100%" align="center" class=borde_tab border="0">
            ...
            <?php    $fecha_desde = date('Y-m-d');
                    echo dibujar_calendario("txt_fecha_inicio_sol", $fecha_desde, __DIR__, ''); ?>
            ...
            <td width="40%" class="listado2">
                <?php    $fecha_desde = date('Y-m-d');
                    echo dibujar_calendario("txt_fecha_fin_sol", $fecha_desde, __DIR__, ''); ?>
