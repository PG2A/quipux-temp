     ...
     <form name="formulario" action="respaldo_solicitud_grabar.php" method="post">        
        ...
        <?php  if($txt_lista_soli_codi){
           foreach ($txt_lista_soli_codi as $idLista=>$valor) {
               // El paginador moderno emite checkValue[] con el codigo en el value; el antiguo lo ponia en la clave.
               $codSoli = is_numeric(trim((string)$valor)) ? trim((string)$valor) : $idLista; ?>
                <input type="hidden" name="txt_lista_soli_codigos[]" id="txt_lista_soli_codi"  value="<?php  echo $codSoli ?>" />
       <?  } }?>
