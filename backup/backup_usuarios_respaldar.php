...
<script language="JavaScript" type="text/JavaScript">
    ...
    function respaldar_documento(resp_codi) {
		...
		//console.log('respaldarDocumento _ contador:'+typeof contador_respaldos[resp_codi][2]);
		if (contador_respaldos[resp_codi][2] == 1) { // Se controla el estado del respaldo: Estado = Ejecutar
            if ((contador_respaldos[resp_codi][1] < contador_respaldos[resp_codi][0]) ) { // Verifica que no existan errores en la ejecución del respaldo
    ...
    function actualizar_barra(resp_codi) { // Controla el movimiento de la barra general y la de cada respaldo
        try {
			...
			document.getElementById('div_barra_'+resp_codi.toString()).style.width=tamano_barra;
            document.getElementById('div_barra_mensaje_'+resp_codi.toString()).innerHTML = '<center>'+ porcentaje + ' %</center>';
            document.getElementById('div_mensaje_'+resp_codi.toString()).innerHTML='<center>Procesando documento No. '+ contador_respaldos[resp_codi][1]+' de '+contador_respaldos[resp_codi][0]+'</center>';
        } catch (e) {
            return;
        }
    }
...
<?php
        ...
        while(!$rs->EOF) { //Creo una barra para cada respaldo
			...
			$tamano_barra_aux = round(200*$procesados /$total);		
?>
                    <tr height="30px">
                        <td>
                        ...
                        <td align="center">
                            <div align='left' style='color: #FFFFFF; height: 20px; width: 206px; border: thin solid #999999; position: relative;'>
                                <div id='div_barra_<?=$rs->fields["RESP_CODI"]?>'
                                     style='background-color: #a8bac6; color: #FFFFFF; border: thin solid #a8bac6;
                                     height:14px; width: <?=$tamano_barra_aux ?>'px; position:absolute; top:2px; left:2px; z-index:1;'></div>
                        ...
                        <td align="center">
                            <img src="../imagenes/play.png" id="img_play_<?=$rs->fields["RESP_CODI"]?>" alt="ejecutar" style="width: 20px; height: 20px;" onclick='ejecutar_respaldo(<?=$rs->fields["RESP_CODI"]?>)'>
                            <img src="../imagenes/pause.png" id="img_pause_<?=$rs->fields["RESP_CODI"]?>" alt="detener" style="width: 20px; height: 20px; display: none;" onclick='pausar_respaldo(<?=$rs->fields["RESP_CODI"]?>)'>
                        </td>
