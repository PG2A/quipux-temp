...
?>
    <script type="text/javascript">
        function cargar_lista_anexos (radi_nume, flag) {
            ...
            nuevoAjax(div, 'GET', '/anexos_lista.php', 'nivel=<?=$nivel_seguridad_documento?>&radi_temp=' + radi_nume + '&fl=' + flag + '&ruta_ajax=.');
        }
        ...
        function verificar_firma(val_path, nomb_arch) {
            ...
            tmp = <?="'/VerificarFirma.php?archivo='"?>;
            URL = <?="'/VerificarFirma.php?archivo='"?> + val_path + '&nombre_archivo=' + nomb_arch;
        ...
        function acciones(anexo, opcion){
            ...
            if (opcion==4)
                if (confirm('El medio de almacenamiento del documento cambiará a electrónico.'))
                    flag=true;
            if (flag) {
                nuevoAjax('div_anexos_accion', 'GET', '/anexos_borrar.php', 'anexo='+anexo+'&accion='+opcion);
...
if ($flag) {
	...
	{
	    echo "</form><form action='".__DIR__."/anexos_funciones.php?nurad=$verrad&textrad=$textrad&krd=$krd' ENCTYPE='multipart/form-data' method='post'>";
	}
...
//	Cargamos los tipos de documentos que se pueden subir al servidor
    ...
    function detectarPhone(){
        ...
        // Mobile
        if (/iphone|ipod|blackberry|android|windows phone|webos|mobile/.test(userAgent))
            return 1;
    ...
    function mostrar_boton(){
        ...
        const isMobile = detectarPhone() === 2;

        // Solo mostrar botón en desktop
        if (!isMobile) {
            ...
            boton.title = 'Graba los anexos que se encuentran en la lista';

            // Insertar en el div
            const div = document.getElementById('div_mostrarboton');
            if (div) {
	...
	for($i=1;$i<=$na;$i++)	//CREO LAS CAJAS DE TEXTO
	{
	    ...
	    $userfile="userfile$i";
	?>
		<tr id="filaanex<?=$i?>" <?php if ($i%2==0) echo 'class="listado2"';?>>
		    <td align="center">
