...
function restaFechas($dFecIni, $dFecFin)
{
    ...
    if (count($aFecIni)!=3) $aFecIni = explode("/", $dFecIni);
    if (count($aFecFin)!=3) $aFecFin = explode("/", $dFecFin);

    // Assuming format YYYY-MM-DD or DD/MM/YYYY based on typical usage, 
    // but code logic below uses indexes [1], [2], [3] which corresponds to ereg matches.
    // ereg matched full string [0] then groups [1][2][3].
    // mktime order: hour, minute, second, month, day, year.
    // original used: mktime(0,0,0,$aFecIni[2], $aFecIni[1], $aFecIni[3]);
    // If input was YYYY-MM-DD/YYYYMMDD, logic is likely:
    // With ereg logic: (group1)(group2)(group3)
    // If format is YYYYMMDD -> G1=MM? No, ereg validation depends on regex.
    // Regex was "([0-9]{1,2})([0-9]{1,2})([0-9]{2,4})" -> 1-2 digits, 1-2 digits, 2-4 digits.
    // This matches DDMMYYYY or MMDDYYYY.
    // Given original: mktime(0,0,0, G2, G1, G3) -> Month=G2, Day=G1, Year=G3.
    // So Input format expected: DD-MM-YYYY (or similar).
    
    // Simplification using DateTime:
    // But keeping logic close to original intents without ereg:
    // Match the regex "([0-9]{1,2})([0-9]{1,2})([0-9]{2,4})"
    preg_match("/([0-9]{1,2})([0-9]{1,2})([0-9]{2,4})/", $dFecIni, $aFecIni);
    preg_match("/([0-9]{1,2})([0-9]{1,2})([0-9]{2,4})/", $dFecFin, $aFecFin);

    if (isset($aFecIni[1]) && isset($aFecFin[1])) {
...
if (isset ($replicacion) && $replicacion && $config_db_replica_buscar_usuario_resultado!="") $db = new ConnectionHandler(__DIR__,$config_db_replica_buscar_usuario_resultado);
include_once(dirname(__DIR__)."/obtenerdatos.php");
include_once(dirname(__DIR__)."/funciones.php");

if (!isset($buscar_inst)) $buscar_inst="0";
if (!isset($buscar_depe)) $buscar_depe="0";
if (!isset($buscar_tipo)) $buscar_tipo="0";
if (!isset($lista_usr)) $lista_usr="0";
$usuarios_lista = "";
$usuarios_lista = "";
$buscar_nom = isset($_POST["buscar_nom"]) ? $_POST["buscar_nom"] : "";
$tipopres = isset($_POST["pres"]) ? $_POST["pres"] : "";
$buscar_car = isset($_POST["buscar_car"]) ? $_POST["buscar_car"] : "";

if (!isset($descCargo)) $descCargo = "Cargo";
if (!isset($ent)) $ent = 0;
...
if ($buscar_nom!="") {
    ...
    $sql = "select u.* $usr_tipo  
            from (select * from usuario where upper(usua_login) 
