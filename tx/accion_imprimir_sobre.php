...
$radi_nume = $radi_nume ?? '';
if (empty($radi_nume) && isset($_POST['checkValue']) && is_array($_POST['checkValue'])) {
    $radi_nume = reset($_POST['checkValue']); // Take the first selected item
}

if (empty($radi_nume)) {
    die(html_error("No se ha seleccionado ning&uacute;n documento."));
}

$datosrad = ObtenerDatosRadicado($radi_nume,$db);
if (empty($datosrad)) {
    die(html_error("No se encontr&oacute; informaci&oacute;n del documento ($radi_nume)."));
}
...
$tmp = str_replace ("-", "", $tmp);
$tmp = trim($tmp, ','); // Remove trailing/leading commas

if (empty($tmp)) {
    // Handle case with no recipients
    $menu_usr = "No existen destinatarios asociados.";
} else {
    ...
    if ($rs && !$rs->EOF) {
        ...
        $menu_usr = "No se encontraron usuarios destinatarios validos.";
    }
}
...
//        texto_sobre = Base64.encode(Base64.encode(oEditor.GetData()));
        ...
        if (oEditor) {
            ...
            texto_sobre = Base64.encode(Base64.encode(document.getElementById('txt_texto_sobre').value));
        }
...
//        oEditor.SetData( editorTexto ) ;
        ...
        if (oEditor) {
            ...
            document.getElementById('txt_texto_sobre').value = editorTexto;
        }
        ...
        <table width="99%" border="0" cellpadding="0" cellspacing="5">
            ...
            <tr>
                <td class='listado1' colspan="3" align="center">
                    ...
                    /*
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <input type='button' value='Regresar' onClick='regresarQuipux();' name='btn_regresar' class='botones_largo' id='btn_regresar' title="Regresa al men&uacute; anterior">
                     */ ?>
                </td>
            </tr>
            <tr>
