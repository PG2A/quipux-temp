...
function obtener_nivel_seguridad_documento($db, $radi_nume) {
    try {
		...
		$sql_comprobar_compartida="select * from bandeja_compartida where usua_codi_jefe=".$rs_rad->fields["RADI_USUA_ACTU"]." and usua_codi=".$_SESSION["usua_codi"]; //carmita
		
		$rs_compartida=$db->query($sql_comprobar_compartida);
        $compartida_jefe = ($rs_compartida && !$rs_compartida->EOF) ? $rs_compartida->fields["USUA_CODI_JEFE"] : null;
        $compartida_usua = ($rs_compartida && !$rs_compartida->EOF) ? $rs_compartida->fields["USUA_CODI"] : null;

        $estado = $rs_rad->fields["ESTA_CODI"];
        $reservado = $rs_rad->fields["RADI_PERMISO"];
        $radi_tipo = substr($radi_nume,-1);
		if ($rs_rad->fields["RADI_USUA_ANTE"]==$_SESSION["usua_codi"]) $usr_anterior=1; else $usr_anterior=0;
        if (($rs_rad->fields["RADI_USUA_ACTU"]==$_SESSION["usua_codi"]) or ($compartida_jefe==$rs_rad->fields["RADI_USUA_ACTU"] and $compartida_usua==$_SESSION["usua_codi"]) ) $usr_actual = 1; else $usr_actual = 0;
		...
		//documento enviado
		if ($estado==6 and $usr_anterior) return 6; //carmita
...
//
        ...
        if (isset($_SESSION["usua_codi_jefe"]) && (int)$_SESSION["usua_codi_jefe"]>0) {
