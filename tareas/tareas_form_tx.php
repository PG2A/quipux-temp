...
if(isset($_POST['checkValue'])) {               //Si se escogieron radicados de la lista
    // Use POST values to avoid integer overflow when very large numeric keys exist
...
<script type="text/javascript">
    ...
    // Verificamos que existan documentos seleccionados
        ...
        lista_radicados = '0';
        for(i=0;i<document.formulario.elements.length;i++) {
            if(document.formulario.elements[i].checked==1 && document.formulario.elements[i].name=='checkValue[]' && document.formulario.elements[i].value!='')
                lista_radicados += ','+document.formulario.elements[i].value;
        }
...
//        document.getElementById('div_ejecutar_tarea').innerHTML = '';
        nuevoAjax('div_ejecutar_tarea'+tarea_ejecutada.toString(), 'POST', '../tareas/tareas_realizar_tx.php',
            'txt_usua_codi='+encodeURIComponent(codigo_usuario)+'&txt_radicados='+encodeURIComponent(lista_radicados)+'&txt_comentario='+encodeURIComponent(comentario)+'&txt_fecha_tarea='+encodeURIComponent(fecha_doc)+'&codTx=<?=$codTx?>');
...
<?php
    ...
    if ($codTx == 30) {  //Buscamos las áreas que se desplegarán en los combos de nueva tarea
            ...
            if(($_SESSION["cargo_tipo"] ?? 0)!=1 && ($_SESSION["usua_publico"] ?? 0) !=1){
...
<?php  }  ?>
            ...
            <tr>
                ...
                <?php  if ($whereFiltro !=="0") { ?>
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                <?php  } ?>
                    <input type='button' value='Regresar' onClick='regresarQuipux();' name='enviardoc' class='botones' id='Cancelar'>
...
<?php
	 ...
	 *  Aqui utilizamos la clase adodb para generar el listado de los radicados
        ...
        $pager = new ADODB_Pager($db->conn,$isql,'adodb', false,1,"");
