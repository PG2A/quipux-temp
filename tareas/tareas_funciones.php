...
function botones_tarea($accion, $tarea_codi, $verrad, $ruta_raiz=".") {
    ...
    switch ($accion) {
        case "nueva":
            $imagen .= "<img src='/iconos/page_new.gif' alt='Nueva tarea' border='0' title='Asignar nueva tarea'";
            $codTx = 30;
            break;
        case "finalizar":
            $imagen .= "<img src='/iconos/page_tick.gif' alt='Finalizar tarea' border='0' title='Finalizar la tarea asignada'";
            $codTx = 31;
            break;
        case "cancelar":
            $imagen .= "<img src='/iconos/page_delete.gif' alt='Cancelar tarea' border='0' title='Cancelar la tarea asignada'";
            $codTx = 32;
            break;
        case "comentar":
            $imagen .= "<img src='/iconos/comment.gif' alt='Comentar tarea' border='0' title='Comentar la tarea'";
            $codTx = 33;
            break;
        case "reabrir":
            $imagen .= "<img src='/iconos/page_refresh.gif' alt='Reabrir tarea' border='0' title='Reabrir tarea cancelada o finalizada'";
            $codTx = 34;
            break;
        case "editar":
            $imagen .= "<img src='/iconos/page_white_edit.png' alt='Editar tarea' border='0' title='Editar tarea'";
            $codTx = 35;
            break;
        case "avance":
            $imagen .= "<img src='/iconos/page_settings.gif' alt='Registrar Avance' border='0' title='Registrar avances en la tarea'";
            $codTx = 36;
            break;
        default :
            return "";
...
//               </div>";
    ...
    switch ($color) {
        case "rojo":
            $color = "#FF0000";
            $titulo = "Vencido";
            $semaforo = "/iconos/semaforo_rojo.gif";
            break;
        case "amarillo":
            $color = "#FFFF00";
            $titulo = "P&oacute;ximo a vencer";
            $semaforo = "/iconos/semaforo_amarillo.gif";
            break;
        case "verde":
            $color = "#01DF01";
            $titulo = "A tiempo";
            $semaforo = "/iconos/semaforo_verde.gif";
            break;
        case "gris":
            $color = "#F2F2F2";
            $titulo = "Finalizado";
            $semaforo = "/iconos/semaforo_gris.gif";
            break;
        default:
            return "";
            break;
    ...
    $imagen = "<img src='$semaforo' alt='$titulo' border='0' title='$titulo' height='25' width='25'>";
...
function dibujar_tareas($db, $tipo_tarea, $verrad, $ruta_raiz=".", $tipo_impresion="") {
    include dirname(__DIR__)."/include/local/localEcuador.php";
    ...
    if ($tipo_tarea == 1) { // Asignadas por mi
        ...
        if ($rst && !$rst->EOF && $rst->fields["ESTADO_PADRE"] == 1) {
    ...
    if ($rst && !$rst->EOF) {
        ...
        while (!$rst->EOF) {
            $tabla .= "<tr><td>".dibujar_detalle_tarea($db, $rst->fields["TAREA_CODI"], ${"botones_e".$rst->fields["ESTADO"]}, $ruta_raiz, $tipo_impresion, $titulo2)."</td></tr>";
            $rst->MoveNext();
        }
        $tabla .= "</table>";
    }
...
function dibujar_detalle_tarea($db, $tarea_codi, $lista_botones, $ruta_raiz=".", $tipo_impresion="", $titulo2="") {
    include dirname(__DIR__)."/include/local/localEcuador.php";
    $estilo_tabla = "style='border: thin solid #377584;'";
    $estilo_font = "color='white' style='font-weight:bold;'";
    $tabla = "";
