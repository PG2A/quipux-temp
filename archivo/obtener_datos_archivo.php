...
//echo $sql;
	...
	if ($nivel>1)
	    $resp .= "<a href='javascript:;' onClick=\"MostrarFila('tr_$codi')\">
			<img src='".__DIR__."/imagenes/add.gif' border='0' alt='' height='15px' width='15px'></a>";
	$resp .= "</td>";
	...
	if ($nivel>1) {
	    ...
	    $resp .= ArbolModificarArchivo($codi, $nivel-1, $nivel_act+1, $dependencia, $nombre2, $db, __DIR__);
	    $resp .= "</table></td></tr>";
	}
	$rs->MoveNext();
    }
...
//echo $sql;
	...
	if ($nivel>1) {
	    ...
	    $resp .= ArbolSeleccionarArchivo($codi, $nivel_act+1, $dependencia,$nombre2,$db,__DIR__,$accion,$campo,$campo_valor,$id, $ocultar);
	    $resp .= "</table></td></tr>";
	}
	$rs->MoveNext();
    }
