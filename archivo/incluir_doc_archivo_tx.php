...
////////////////	VARIABLES BASICAS	////////////////////////
  ...
  $radicados = "";
  $chequeados = $_POST["checkValue"] ?? array();
  foreach($chequeados as $k => $v) {
	// Support both old keyed format (key=radicado, value=CHKANULAR) and new value-array format
	if ($v === 'CHKANULAR' || $v === 'chk_copiar_anexos') {
		$rad = $k;
	} else {
		$rad = $v;
	}
	if (trim($rad) != "") $radicados .= trim($rad).",";
  }
...
?>
  ...
  <form method="post" name="formulario" action="./incluir_doc_archivo_grabar.php"> 
	...
	<tr><td  colspan="3">
	    <table width="100%">
		<?php echo ArbolSeleccionarArchivo(0, 0 , $depe_archivo, "", $db, __DIR__,"S","E",1);?>
	    </table></td>
	</tr>
    </table>
    <br>
