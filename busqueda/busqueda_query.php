...
switch($db->driver)
{
    case 'postgres':
        $nestloop = "off";
        if (!isset($txt_tipo_fecha)) $txt_tipo_fecha = "radi_fech_ofic";
        $tipo_fecha = "radi_fech_ofic";
        if ($txt_tipo_fecha == "1") $tipo_fecha = "radi_fech_radi";
        $sqlFecha = "substr($tipo_fecha::text,1,19)";

        $txt_inst_codi = (isset($_SESSION["perm_buscar_doc_adscritas"]) && $_SESSION["perm_buscar_doc_adscritas"]==1) ? 0+limpiar_numero($txt_inst_codi) : $_SESSION["inst_codi"];

        $txt_fecha_desde = limpiar_sql($txt_fecha_desde);
        $txt_fecha_hasta = limpiar_sql($txt_fecha_hasta);
        
        // Fix clean_sql if used anywhere else, but above lines cover explicit calls.
        // Also checking for potential clean_sql in later lines... regex replace might be safer if multiple instances.
	...
	$where_ext = " (r.esta_codi!=6 or mod(r.radi_nume_radi,10)<>1)";
        ...
        if (trim($txt_tipo_documento) != "0")
            $where .= " and r.radi_tipo=".(0 + (limpiar_numero($txt_tipo_documento) ?: 0));

        if (trim($txt_categoria) != "")
            $where_ext .= " and coalesce(r.cat_codi,0)=".(0 + (limpiar_numero($txt_categoria) ?: 0));
        if (trim($txt_tipificacion) != "")
            $where_ext .= " and coalesce(r.cod_codi,0)=".(0 + (limpiar_numero($txt_tipificacion) ?: 0));
        ...
        $txt_estado = (trim($txt_estado ?? '') === '') ? 999 : (0 + limpiar_numero($txt_estado));
        if ($txt_estado==999)
            $where .= " and r.esta_codi in (0,1,2,3,6,7,9)";
        else
            $where .= " and r.esta_codi = ".$txt_estado;
            
        $txt_sino_firma = (trim($txt_sino_firma ?? '') === '') ? 2 : (0 + limpiar_numero($txt_sino_firma));
        if ($txt_sino_firma==0)
        ...
        $isql = "select -- Busqueda Avanzada - USR ".$_SESSION["usua_codi"]." - ".date("Y-m-d H:i:s")." - nestloop=$nestloop<br>
            ...
            ,$sqlFecha || '" . (isset($descZonaHoraria) ? $descZonaHoraria : '') . "' as \"DAT_Fecha Documento\"
		...
		$isql.=" and r.esta_codi=7 then 'Anulado' else es.esta_desc end as esta_desc
            ...
            $from
            where $where_ext
            order by " . ($orderNo+1) . " $orderTipo, fecha_orden asc
        ) as a";
