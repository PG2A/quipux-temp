...
$perm_adscritas = $_SESSION["perm_buscar_doc_adscritas"] ?? 0;
$tipo_combo = isset($_POST["tipo_combo"]) ? limpiar_sql($_POST["tipo_combo"]) : "";

if ($tipo_combo=="inst" && $perm_adscritas!=1) die("");

$txt_inst_codi = ($perm_adscritas==1) ? (int)limpiar_numero($_POST["txt_inst_codi"] ?? 0) : ($_SESSION["inst_codi"] ?? 0);
...
if($rs && !$rs->EOF)
    print $rs->GetMenu2("slc_$tipo_combo"."_combo", ${"txt_$tipo_combo"."_codi"}, $default, false, 0, "class='select' id='slc_$tipo_combo"."_combo' onChange='fjs_buscar_cargar_combo(\"$tipo_combo\", this);'");
?>
