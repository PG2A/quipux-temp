...
if (isset ($replicacion) && $replicacion && $config_db_replica_busqueda_tramites!="") {
    $db = new ConnectionHandler(__DIR__, $config_db_replica_busqueda_tramites);
}
...
$ruta_raiz = "..";
if (!isset($descRadicado)) $descRadicado = "Radicado";
if (!isset($descReferencia)) $descReferencia = "Referencia";
if (!isset($descDependencia)) $descDependencia = "Dependencia";
$numeroCaracteresTexto = 0;

$txt_nume_documento  = limpiar_sql(isset($_POST["txt_nume_documento"]) ? $_POST["txt_nume_documento"] : "");
$txt_nume_referencia = limpiar_sql(isset($_POST["txt_nume_referencia"]) ? $_POST["txt_nume_referencia"] : "");

$txt_usua_remitente  = limpiar_sql(isset($_POST["txt_usua_remitente"]) ? $_POST["txt_usua_remitente"] : "");
$txt_usua_destinatario  = limpiar_sql(isset($_POST["txt_usua_destinatario"]) ? $_POST["txt_usua_destinatario"] : "");

$txt_depe_codi  = limpiar_sql(isset($_POST["txt_depe_codi"]) ? $_POST["txt_depe_codi"] : "");
$txt_usua_codi  = limpiar_sql(isset($_POST["txt_usua_codi"]) ? $_POST["txt_usua_codi"] : "");
$txt_texto  = limpiar_sql(isset($_POST["txt_texto"]) ? $_POST["txt_texto"] : "");
...
<script language="JavaScript" type="text/javascript" >
    <?php  include_once("../js/spiffyCal/spiffyCal_v2_1.js");?>
    ...
    function cargar_combo_usuarios(txt_usua_codi) {
        var obj = document.getElementById("txt_depe_codi");
        area = (obj) ? obj.value : 0;
    ...
    {
        var_envio='../verradicado.php?verrad='+radi_nume+'&textrad='+radi_text+'&menu_ver_tmp=3&tipo_ventana=popup';
        ...
        <table width="90%" class="borde_tab">
            ...
            <tr>
                ...
                if($rs && !$rs->EOF)
                    ...
                    echo "<input type='hidden' name='txt_depe_codi' id='txt_depe_codi' value='0'>";
                ?>
                </td>
                <td width="30%" class="listado2">&nbsp;
                </td>
