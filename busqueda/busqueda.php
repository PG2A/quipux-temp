...
session_start();
if (isset ($replicacion) && $replicacion && $config_db_replica_busqueda!="") {
    $db = new ConnectionHandler(__DIR__, $config_db_replica_busqueda);
}
...
$config_numero_meses = $config_numero_meses ?? 3;
$rad_nume_docu_exacto = $_POST['rad_nume_docu_exacto'] ?? 0;

echo "<!DOCTYPE html>".html_head();
include_once('../js/ajax.js');

$paginador = new ADODB_Pager_Ajax(dirname(__DIR__), "div_buscar_documentos", "busqueda_paginador.php",
...
if (isset($_POST["txt_inst_codi"]) && ($_SESSION["perm_buscar_doc_adscritas"] ?? 0) == 1 && (0+$_POST["txt_inst_codi"])!=$_SESSION["inst_codi"]) {
...
if (isset($_POST["txt_depe_codi"])) {
    $txt_depe_codi = (int)limpiar_numero($_POST["txt_depe_codi"] ?? 0);
    $txt_usua_codi = (int)limpiar_numero($_POST["txt_usua_codi"] ?? 0);
    $txt_fecha_desde = limpiar_sql($_POST["txt_fecha_desde"] ?? '');
    $txt_fecha_hasta = limpiar_sql($_POST["txt_fecha_hasta"] ?? '');
} else {
    $txt_depe_codi = $_SESSION["depe_codi"] ?? 0;
    $txt_usua_codi = $_SESSION["usua_codi"] ?? 0;
    if (($_SESSION["ver_todos_docu"] ?? 0)==1) { // Si tiene permiso de bandeja de entrada coja todas las areas por defecto
        $txt_depe_codi = 0;
        $txt_usua_codi = 0;
    }
...
$txt_buscar = (int)($_POST["txt_buscar"] ?? 0);

if ($txt_depe_codi==($_SESSION["depe_codi"] ?? 0)) $span_depe_nombre = $_SESSION["depe_nomb"] ?? '';
elseif ($txt_depe_codi == 0) $span_depe_nombre = "&lt;&lt Todas las &aacute;reas &gt;&gt;";
    ...
    $span_depe_nombre = ($rs && !$rs->EOF) ? $rs->fields["DEPE_NOMB"] : '';
}
...
if ($txt_usua_codi == 0) $span_usua_nombre = "&lt;&lt Todos los usuarios &gt;&gt;";
    ...
    $span_usua_nombre = ($rs && !$rs->EOF) ? $rs->fields["USUA_APELLIDO"]." ".$rs->fields["USUA_NOMB"] : '';
}
...
Buscar cualquier coincidencia: Busca cualquier coincidencia entre el n&uacute;mero del documento y el texto ingresado.">
		...
		<td  align="left" class="titulos2"><span class="titulos2">Estado del Documento: </span></td>
             ...
             <?php
                ...
                echo $rsE->GetMenu2("txt_estado", $txt_estado, "999:&lt;&lt Todos &gt;&gt;", false, 0, " id='txt_estado' class='select'" );
            ...
            <tr>
                ...
                <td class="listado2">
<?php
                    $rs = $db->conn->Execute("select trad_descr, trad_codigo from tiporad where trad_inst_codi in (0,".($_SESSION["inst_codi"] ?? 0).") order by 2");
                    echo $rs->GetMenu2("txt_tipo_documento", "$txt_tipo_documento", "0:&lt;&lt Todos &gt;&gt;", false, 0, "class='select' id ='txt_tipo_documento'");
?>
                </td>
                <td width="30%" class="listado2">&nbsp;
                    &nbsp;
...
<?php
                ...
                if($rs && !$rs->EOF)
                    print $rs->GetMenu2("txt_categoria", "$txt_categoria", ":&lt;&lt Seleccione Categor&iacute;a &gt;&gt;", false, 0, "class='select' id='txt_categoria'" );
?>
                </td>
                <td width="30%" class="listado2">&nbsp;
                </td>
        ...
        if(!$rsCod->EOF)
        {
            echo "
                <tr>
                    ...
                    <td class='listado2'>";
                    print $rsCod->GetMenu2("txt_tipificacion", "$txt_tipificacion", ":&lt;&lt Seleccione Tipificaci&oacute;n &gt;&gt;", false, 0, 
...
?>
            ...
            <tr style='display: <?= (($_SESSION["perm_buscar_doc_adscritas"] ?? 0) == 1) ? "" : "none";?>;'>
