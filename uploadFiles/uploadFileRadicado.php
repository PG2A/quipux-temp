...
if (isset ($replicacion) && $replicacion && $config_db_replica_uf_upload_file_radicado!="") {
    $db = new ConnectionHandler(__DIR__, $config_db_replica_uf_upload_file_radicado);
}
...
function mostrar_documento(numdoc, txtdoc,carpeta)
    {
	var_envio='../verradicado.php?verrad='+numdoc+'&textrad='+txtdoc+'&carpetad='+carpeta+'&menu_ver_tmp=3';
...
//include __DIR__."/envios/paEncabeza.php";
$pagina_actual = "uploadFileRadicado.php";
include_once(dirname(__DIR__)."/envios/paBuscar.php");
...
//		$orderTipo=" DESC ";
    ...
    if (trim($imprimir)=="si")
        $query = $query1;
    $pager = new ADODB_Pager($db->conn,$query,'adodb', true,$orderNo,$orderTipo);
