...
if (isset ($replicacion) && $replicacion && $config_db_replica_uf_cargar_doc_digitalizado!="") {
    $db = new ConnectionHandler(__DIR__, $config_db_replica_uf_cargar_doc_digitalizado);
}
include_once(dirname(__DIR__).'/obtenerdatos.php');
include_once(dirname(__DIR__).'/funciones_interfaz.php');
include_once(dirname(__DIR__)."/js/calendario_php/calendario_php.php");
...
//paginador
$paginador = new ADODB_Pager_Ajax(dirname(__DIR__), "div_cuerpo", "cargar_doc_digitalizado_paginador.php",
...
$txt_depe_codi = (int)($_GET['txt_depe_codi'] ?? 0);
$txt_usua_codi = $_GET['txt_usua_codi'] ?? 0;
if (isset($_GET['txt_fecha_desde']) && $_GET['txt_fecha_desde']!='')
$txt_fecha_desde = limpiar_sql ($_GET['txt_fecha_desde']);
if (isset($_GET['txt_fecha_hasta']) && $_GET['txt_fecha_hasta']!='')
$txt_fecha_hasta = limpiar_sql ($_GET['txt_fecha_hasta']);
$busqRadicados = '';
if (isset($_GET['busqRadicados']) && $_GET['busqRadicados']!='')
$busqRadicados = limpiar_sql ($_GET['busqRadicados']);
$imprimir = $_GET['imprimir'] ?? '';
if (trim((string)$imprimir)=="si")
	$accion= "../plantillas/CodigoBarras.php?nuevo=no";
else
//	$accion="formUpload.php";
	$accion= "../anexos/cargar_imagen_digitalizada.php";
...
{
    var_envio='../verradicado.php?verrad='+numdoc+'&textrad='+txtdoc+'&carpetad='+carpeta+'&menu_ver_tmp=3&tipo_ventana=popup';
