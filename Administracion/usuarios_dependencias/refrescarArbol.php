...
function obtenerAreas($codInst, $db,$usrCodigo)
{
    ...
    while(!$rsDepePadre->EOF)
    {
        ...
        $existe=obtenerCodigos($usrCodigo,$rsDepePadre->fields["DEPE_CODI"],$db,1);
        //usuario que da permisos
        $existeAdmin=obtenerCodigos($usua_admin,$rsDepePadre->fields["DEPE_CODI"],$db,1);//pertenece a administrador        
...
function obtenerDependencia($depe_codi, $db,$usrCodigo){
    ...
    while(!$rsDepeHijo->EOF)
    {
           ...
           $depe_codi_fin=obtenerCodigos($usrCodigo,$rsDepeHijo->fields["DEPE_CODI"],$db,1);
            
           if (obtenerCodigos($usrCodigo,$rsDepeHijo->fields["DEPE_CODI"],$db,1)==1)//si ya administra el usuario la dependencia
        ...
        $existe=obtenerCodigos($usrCodigo,$rsDepeHijo->fields["DEPE_CODI"],$db,1);
        
        $existeAdmin=obtenerCodigos($usua_admin,$rsDepeHijo->fields["DEPE_CODI"],$db,1);//pertenece a administrador        
...
function graficacheck($opcion,$depe_codi,$db){
    if (countPadre($depe_codi,$db,1)==1)//si tiene hijos
