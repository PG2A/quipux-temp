...
if (!isset($_SESSION["descDependencia"])) {
    $_SESSION["descDependencia"] = "Dependencia";
}
...
if ($txt_depe_codi != 0) {
    ...
    $fields = array_change_key_case($rs->fields, CASE_UPPER);
    $txt_nombre = $fields['DEPE_NOMB'];
    $txt_sigla = $fields['DEP_SIGLA'];
    $slc_padre = $fields['DEPE_CODI_PADRE'];
    $txt_ciudad = $fields['DEPE_PIE1']; //
    $slc_archivo = $fields['DEP_CENTRAL'];
    $slc_plantilla = $fields['DEPE_PLANTILLA'];
    $depe_estado = $fields['DEPE_ESTADO'];
    if (isset($fields['INST_ADSCRITA']) && trim($fields['INST_ADSCRITA']) != "") $where_adscrita = " and inst_adscrita=".trim($fields['INST_ADSCRITA']);
}
else {
    $txt_nombre = "";
    $txt_ciudad = "";
    $txt_sigla = "";
    $depe_estado = 1; // Default state for new dependency
    $slc_padre = isset($_GET['padre']) ? 0+$_GET['padre'] : 0;
    $slc_archivo = 0;
    $slc_plantilla = 0;
}
...
<?php  if ($accion==1 or $slc_dependencia!=0) { ?>
  ...
  <?php 
    ...
    $editar_area = obtenerCodigos($_SESSION['usua_codi'],$txt_depe_codi,$db,1);
  ...
  <table width="100%" class="borde_tab">      
	...
	<td class="listado2_ver"><?php
        ...
        echo "<select name='txt_ciudad' class='select'>";
        echo "<option value='0'>&lt;&lt seleccione &gt;&gt;</option>";
        while (!$rs->EOF) {
            $val = isset($rs->fields['ID']) ? $rs->fields['ID'] : $rs->fields[1];
            $text = isset($rs->fields['NOMBRE']) ? $rs->fields['NOMBRE'] : $rs->fields[0];
            $selected = ($val == $txt_ciudad) ? " selected" : "";
	...
	<?php
	    ...
	    $sql.= " and depe_codi not in (".substr(buscar_areas_dependientes_rec($txt_depe_codi),1).")";
            ...
            $rs=$db->conn->query($sql);
        echo "<select name='slc_padre' class='select'>";
        echo "<option value='0'>&lt;&lt Área Actual &gt;&gt;</option>";
        while (!$rs->EOF) {
            $val = isset($rs->fields['DEPE_CODI']) ? $rs->fields['DEPE_CODI'] : $rs->fields[1];
            $text = isset($rs->fields['DEPE_NOMB']) ? $rs->fields['DEPE_NOMB'] : $rs->fields[0];
            $selected = ($val == $slc_padre) ? " selected" : "";
            echo "<option value='$val'$selected>$text</option>";
            $rs->MoveNext();
        }
        echo "</select>";
	...
	<?php
	    ...
	    $rs=$db->conn->query($sql);
        echo "<select name='slc_archivo' class='select'>";
        echo "<option value='0'>&lt;&lt Área Actual &gt;&gt;</option>";
        while (!$rs->EOF) {
            $val = isset($rs->fields['DEPE_CODI']) ? $rs->fields['DEPE_CODI'] : $rs->fields[1];
            $text = isset($rs->fields['DEPE_NOMB']) ? $rs->fields['DEPE_NOMB'] : $rs->fields[0];
            $selected = ($val == $slc_archivo) ? " selected" : "";
            echo "<option value='$val'$selected>$text</option>";
            $rs->MoveNext();
        }
        echo "</select>";
	...
	<?php
	    $sql = "select depe_nomb, depe_codi from dependencia where depe_estado=1 $where_adscrita and (coalesce(depe_plantilla,depe_codi)=depe_codi
		    or depe_codi=".$_SESSION["depe_codi"].") and inst_codi=".$_SESSION["inst_codi"]." order by 1";
	    $rs=$db->conn->query($sql);
        echo "<select name='slc_plantilla' class='select' id='slc_plantilla' onChange='SeleccionarPlantilla(\"".$_GET['dependencia']."\")'>";
        echo "<option value='0'>&lt;&lt Área Actual &gt;&gt;</option>";
        while (!$rs->EOF) {
            $val = isset($rs->fields['DEPE_CODI']) ? $rs->fields['DEPE_CODI'] : $rs->fields[1];
            $text = isset($rs->fields['DEPE_NOMB']) ? $rs->fields['DEPE_NOMB'] : $rs->fields[0];
            $selected = ($val == $slc_plantilla) ? " selected" : "";
            echo "<option value='$val'$selected>$text</option>";
            $rs->MoveNext();
        }
        echo "</select>";
...
<?php
	if ($slc_plantilla==0 or $slc_plantilla==$slc_dependencia) {
	    if (is_file(dirname(__DIR__, 2)."/bodega/plantillas/$slc_dependencia.pdf")) {
		$path_descarga = "../../archivo_descargar.php?path_arch=/plantillas/$slc_dependencia.pdf&nomb_arch=plantilla.pdf";
...
?>
	    ...
	    <br><b>La plantilla debe estar en formato &quot;pdf&quot;, una p&aacute;gina tama&ntilde;o A4.</b>
	</td>
    </tr>
  </table>
