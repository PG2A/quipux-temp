...
require_once(dirname(__DIR__, 2).'/include/portal.php');

p_register_globals(array());

$inst_estado=0;
if (isset($_REQUEST['slc_institucion']) and $_REQUEST['slc_institucion'] !== '') {
    $slc_institucion_pedida = (int)$_REQUEST['slc_institucion'];
    if ($_SESSION["admin_institucion"]==1 or $slc_institucion_pedida == (int)$_SESSION["inst_codi"]) {
        $slc_institucion = $slc_institucion_pedida;
    }
}
...
if (isset($_POST['btn_accion'])) {
	...
	$record['INST_ESTADO'] = "1";
        if(!isset($_POST['chkCoordinador'])) $_POST['chkCoordinador'] = '0';
        ...
        $arch_logo = "";
        if (isset($_FILES["arch_logo"]) && $_FILES["arch_logo"]['error'] == UPLOAD_ERR_OK) {
	...
	if (trim($arch_logo!="")) {
	    ...
	    $ok2 = move_uploaded_file($arch_logo,dirname(__DIR__, 2).$path_arch);
	...
	if($ok1==2){
       ...
       // Creamos el área padre de la institución
        ...
        if (!mkdir($concurrentDirectory = dirname(__DIR__, 2) . "/bodega/" . date('Y') . "/" . $dependencia . "/docs", 0777, true) && !is_dir($concurrentDirectory)) {
	...
	if ($ok1 && $ok2 && $ok3) {
	    ...
	    portal_guardar_visibilidad($db->conn, $txt_id_inst, isset($_POST['chkMostrarPortal']), (int)$_SESSION['usua_codi']);
...
if ($slc_institucion != 0) {
    ...
    $chkMostrarPortal = portal_institucion_visible($db->conn, $slc_institucion) ? 1 : 0;
...
//    $txtPie3 = $rs->fields['INST_PIE3'];
    ...
    if($rsCoordinador && !$rsCoordinador->EOF){
        $siCoor = 'S';
    }
    else
        $siCoor = 'N';
...
<?php require_once(dirname(__DIR__, 2)."/js/ajax.js"); ?>
...
{	
	...
	if (ltrim(document.formSeleccion.txtSigla.value) == '')
	{
        ...
        var esEmailEl = document.getElementById("esEmail");
        if (esEmailEl && ltrim(esEmailEl.value)==0){
...
//	window.open('listados.php?var='+inst,'','scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
	window.open('lista_jquery.php?tipo_reporte=instituciones','listado_instituciones','scrollbars=yes,menubar=no,height=600,width=800,resizable=yes,toolbar=no,location=no,status=no');
}
...
<?php } else $slc_institucion = $_SESSION["inst_codi"]; ?>
  ...
  <table width="100%"class="borde_tab">
    ...
    <tr>
        ...
        <td class="listado2" width="30%" colspan="3"><input type="checkbox" name="chkCoordinador" id="chkCoordinador" value="<?=$chkCoordinador?>" <?php  if($chkCoordinador=='1') echo 'checked'?> onclick="coordinador();" <?php echo $read; ?>></td>
        <?php } ?>
    </tr>
    <tr>
...
<?php if (portal_tiene_tabla_visibilidad($db->conn)) { ?>
    <tr>
	<td class="titulos2"><b>Mostrar en la pantalla de inicio:</b></td>
	<td width="35%" class="listado2" colspan="3">
	    <input type="checkbox" name="chkMostrarPortal" id="chkMostrarPortal" value="1" <?php if (!isset($chkMostrarPortal) or $chkMostrarPortal == 1) echo 'checked'; ?> <?php echo $read; ?>>
	    <label for="chkMostrarPortal">Aparece en el men&uacute; &quot;Ingresar a Quipux&quot; del portal p&uacute;blico</label>
	</td>
	<td width="35%" class="listado2"></td>
    </tr>
<?php } ?>
    <tr>
	...
	<td width="35%" class="listado2" colspan="2">
        ...
        <td width="35%" class="listado2" colspan="2">
	    <?php
		$ruta_logo = trim($txtLogo);
		$ruta_logo_web = $ruta_logo !== '' ? '/' . ltrim($ruta_logo, './') : '';
		$ruta_logo_disco = $ruta_logo !== '' ? dirname(__DIR__, 2) . '/' . ltrim($ruta_logo, './') : '';
		if ($ruta_logo_web !== '' && is_file($ruta_logo_disco)) { ?>
	    	<center><img src="<?=$ruta_logo_web?>" width="180" height="60" border=0 alt="Logo Instituci&oacute;n"></center>
	    <?php  } ?>
	</td>
    </tr>
<?php  if ($_SESSION["usua_codi"]==0) { ?>
	...
	<td width="35%" class="listado2">
	    <input type="checkbox" name="chkCoordinador" id="chkCoordinador" value="<?=$chkCoordinador?>" <?php  if($chkCoordinador=='1') echo 'checked'?> onclick="coordinador();">
	</td-->
