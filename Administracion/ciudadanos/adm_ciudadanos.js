...
function validar_cambio_cedulajs(cedula) {
    ...
    var js_usr_codigo = 0;
    if (document.getElementById('usr_codigo')) {
        js_usr_codigo = document.getElementById('usr_codigo').value;
    }

    nuevoAjax('div_datos_registro_civil', 'POST', '../usuarios/validar_datos_registro_civil.php', 'cedula=' + cedula);
    nuevoAjax('div_datos_usuario_multiple', 'POST', '../usuarios/validar_datos_usuario_multiple.php', 'usr_codigo=' + js_usr_codigo + '&cedula=' + cedula);
...
function ver_datos(tipo) {
    var obj;
    if (tipo == 2)
        obj = document.getElementById('old_cedula');
    else
        obj = document.getElementById('new_cedula');

    // Fallback: if primary element doesn't exist, try 'usr_cedula' (used in adm_usuario.php)
    if (!obj) {
        obj = document.getElementById('usr_cedula');
    }
    ...
    if (obj) {
        ...
        if (cedula != '') {
            ...
            if (document.getElementById('div_datos_registro_civil'))
                document.getElementById('div_datos_registro_civil').style.display = '';
        }
        else {
