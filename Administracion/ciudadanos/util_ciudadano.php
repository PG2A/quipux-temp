...
class Ciudadano {
    ...
    function __construct($db = null) {
        if ($db instanceof ConnectionHandler) {
            // Se pasó el manejador completo, extraemos la conexión real
            $this->db = $db->conn;
            $this->ruta_raiz = realpath(__DIR__ . "/../..");
        } elseif ($db instanceof ADOConnection) {
            // Se pasó la conexión ADOdb directamente
            $this->db = $db;
            $this->ruta_raiz = realpath(__DIR__ . "/../..");
        } else {
            // No se pasó nada, crear conexión nueva
            $this->ruta_raiz = realpath(__DIR__ . "/../..");
            require_once $this->ruta_raiz . "/include/db/ConnectionHandler.php";
            $connHandler = new ConnectionHandler($this->ruta_raiz, "");
            $this->db = $connHandler->conn;
        }

        // Validación final
        if (!($this->db instanceof ADOConnection)) {
            $tipo = is_object($this->db) ? get_class($this->db) : gettype($this->db);
            die("Error en Ciudadano: la propiedad db no es ADOConnection. Tipo detectado: $tipo");
        }
    }
...
//    }
    ...
    private function execQuery($sql) {
        if (!($this->db instanceof ADOConnection)) {
            throw new Exception("Error: conexión ADOdb inválida en Ciudadano");
        }
    ...
    ***********************************************************************************/
        ...
        ***********************************************************************************/
        function comparar_campo_ciudad($campo, $new_value,$old_value,$label, $tamano, $opciones="") {
            ...
            echo "<tr>
                    ...
                    <td class='tooltip titulos3'><center>
        ...
        ***********************************************************************************/
        function comparar_campo($campo, $new_value, $old_value, $label, $tamano, $opciones="") {
            ...
            $cad.="<td width='10%' class='tooltip titulos3'><center><input type='button' name='btn_accion' class='botones_2' value='&gt;&gt;' onclick=\"mover_dato('$campo')\"/ title='De click para mover los datos..'>
            </center></td>";        
        ...
        *********************************************************************************************/
        function mostrar_div($nombre_div,$titulo='',$tabla='si',$imagen=''){
            ...
            //if ($imagen=='mas'){
            if ($imagen=='menos'){
                $html.="<img id='".$nombre_div."img_menos' name='".$nombre_div."img_menos' src='/iconos/menos.gif' border='1' title='Ver Consulta' onclick='$fnjava' alt='Ver Consulta'/>";
                $html.="<img id='".$nombre_div."img_mas' name='".$nombre_div."img_mas' src='/iconos/mas.gif' style='display:none' title='Ver Consulta' border='1' onclick='$fnjava' alt='Ver Consulta'/>";            
            }else{
                $html.="<img id='".$nombre_div."img_menos' name='".$nombre_div."img_menos' src='/iconos/menos.gif' style='display:none' border='1' title='Ver Consulta' onclick='$fnjava' alt='Ver Consulta'/>";
                $html.="<img id='".$nombre_div."img_mas' name='".$nombre_div."img_mas' src='/iconos/mas.gif' title='Ver Consulta' border='1' onclick='$fnjava' alt='Ver Consulta'/>";            
            }
            $html.="</a> $titulo";
             if ($tabla=='si')
            $html.="</td></tr></table>";
        ...
        *********************************************************************************************/
        function verHistorico($usr_ciud_codi,$tipo,$tabla_modificada,$nombre_div=''){
             ...
             $html.="<a href='javascript:void(0);' class='vinculos' target='mainFrame'>";            
             $html.="<img id='".$nombre_div."img_mas' name='".$nombre_div."img_mas' src='/iconos/mas.gif'  border='1' title='Ver Consulta' onclick='$fnjava' alt='Ver Consulta'/>";
             $html.="<img id='".$nombre_div."img_menos' name='".$nombre_div."img_menos' src='/iconos/menos.gif' style='display:none' title='Ver Consulta' border='1' onclick='$fnjava' alt='Ver Consulta'/>";            
        ...
        function grabar_log_tabla($tabla,$rs_old, $rs_new, $usua_codi_actualiza,$tabla_modificar) {
            ...
            foreach ($rs_new->fields as $campo => $valor) {
                if ($rs_old->EOF) { // Si es nuevo registro
                    if (trim((string)$valor) != "") { // Se se puso un valor inicial diferente de null   
   ...
   function graficarGeo($ciu_pais,$ciu_provincia,$ciu_ciudad,$ciu_canton,$direccion,$referencia){
      ...
      <input type='hidden' id='hcod_canton' name='hcod_canton' value='$ciu_canton'/>";
       ...
       $rsCmbPais = $this->execQuery($sqlCmbPais);
   ...
   function dibujarCiudad($ciudad){
       ...
       //echo $sql;
        ...
        $rsDepePadre = $this->execQuery($sql);
    ...
    */
    ...
    function guardar_permisos($usua_codi,$id_permiso,$accion){
        $rec=array();
        $record=array();
        $rec["FECHA_ACTUALIZA"] = $this->db->sysTimeStamp;
        ...
        if ($accion){
            $this->db->Replace("PERMISO_USUARIO", $record, "", false,false,true,false);
            $this->db->Replace("LOG_USR_PERMISOS", $rec, "", false,false,false,false);
        }
        else{
