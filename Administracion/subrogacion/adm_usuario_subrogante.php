...
if(isset($_GET['usr_subrogante']) && trim($_GET['usr_subrogante'] ?? '') != "")
$usr_codigo = 0 + $_GET['usr_subrogante'];
else
$usr_codigo = 0;
...
// Initialize Subrogante variables to empty defaults
$usr_depe = ""; $area_inicio = ""; $usr_perfil = ""; $perfil_inicio = "";
$usr_login = ""; $usr_cedula = ""; $usr_nombre = ""; $usr_apellido = "";
$usr_titulo = ""; $usr_abr_titulo = ""; $usr_cargo = ""; $usr_cargo_cabecera = "";
$usr_sumilla = ""; $usr_inst_nombre = ""; $usr_responsable_area = ""; $cargo_id = "";
$usr_email = ""; $usr_obs = ""; $codi_ciudad = ""; $usr_nuevo = "checked";
$usr_estado = "checked"; $usr_firma_path = ""; $usr_direccion = ""; $usr_telefono = "";
$usr_codi_actualiza = ""; $usr_fecha_actualiza = ""; $usr_obs_actualiza = "";
$checked_tipo_identificacion = ""; $usr_ciudad = "";
...
if ($usr_codigo > 0) {
    ...
    if ($rs && !$rs->EOF) {
        ...
        $usr_responsable_area = (!empty($rs->fields["USUA_RESPONSABLE_AREA"]) && $rs->fields["USUA_RESPONSABLE_AREA"] == 1)? "checked" : "";
        $cargo_id       = $rs->fields["CARGO_ID"] ?? "";
        $usr_email      = $rs->fields["USUA_EMAIL"] ?? "";
        $usr_obs        = $rs->fields["USUA_OBS"] ?? "";
        $codi_ciudad    = $rs->fields["CIU_CODI"] ?? "";
        $usr_nuevo      = (!empty($rs->fields["USUA_NUEVO"]) && $rs->fields["USUA_NUEVO"] == 1) ? "" : "checked";
        $usr_estado     = (empty($rs->fields["USUA_ESTA"]) || $rs->fields["USUA_ESTA"] == 0) ? "" : "checked";
        ...
        $checked_tipo_identificacion = (!empty($rs->fields["TIPO_IDENTIFICACION"])) ? "checked" : "";
    }
}
        //PARA SUBROGANTE
...
<?php  echo "<!DOCTYPE html>".html_head(); /*Imprime el head definido para el sistema*/?>
  ...
  <form name='frmCrear' action="grabar_usuario_subrogante.php" method="post" ENCTYPE='multipart/form-data'>
   ...
   <input type="hidden" name="fecha_hoy" id="fecha_hoy" value="<?=$fecha_tarea?>" />
    ...
    <table width="100%" border="1" align="center" class="t_bordeGris" id="usr_datos">
        ...
        <tr>
            <td class="titulos2" width="20%"> <?php echo ($_SESSION["inst_codi"]==1) ? ($_SESSION["descEmpresa"] ?? 'Institución') : "* ".($_SESSION["descDependencia"] ?? 'Área');?></td>
            <td class="listado2" width="30%">
            <?php
            echo $depNombJefe;
            ?>
...
<!--                <input type="checkbox" name="usr_area_responsable" id="usr_area_responsable" value="0" onclick="Obtener_val(this)" title="Iniciales del usuario que se visualizará el pie de página (mayùculas) de un documento" <?php echo $usr_responsable_area ."  " .$read; ?>>Responsable de Área-->
        ...
        <td width="15%" class="titulos2">Desde Fecha (yyyy/mm/dd):</td>
                    ...
                    echo dibujar_calendario("txt_fecha_desde", $fecha_desde,"../..", "validar_fecha_maxima();"); ?>
                    </td>
                    ...
                    $fecha_hasta = date('Y-m-d');
                      echo dibujar_calendario("txt_fecha_hasta", $fecha_hasta, "../..", "validar_fecha_maxima();"); ?>
                    </td>
        </tr>
