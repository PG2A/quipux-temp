...
if (!isset($recargar)) {
    ...
    // Fix: Ensure inst_codi is cast to int for safety
    $inst_codi = (int)($_SESSION['inst_codi'] ?? 0);
    
    // Fix: Handle case sensitivity and potential nulls
    $sql = "select u.usua_codi, u.usua_nombre, u.usua_cedula, u.inst_codi  
            from usuario u 
            where coalesce(u.depe_codi,-1)=-1 
            and u.usua_esta=1 
            and u.inst_codi =$inst_codi 
    ...
    if (!$rsusuarios) {
        ...
        echo "<div class='advice'>Error al consultar usuarios: $error <br> SQL: $sql</div>";
        $rsusuarios = new ADORecordSet_empty();
    }

    $depe_codi_admin = obtenerAreasAdmin($_SESSION["usua_codi"], $inst_codi, $_SESSION["usua_admin_sistema"], $db);
    
    // Fix: Dependencia table might need lowercase or specific case depending on DB
    $sql2 = "select * from dependencia d where d.depe_estado=1 and d.inst_codi =$inst_codi";
    
    // Fix: PHP 8 comparison check to avoid empty IN ()
    if (!empty($depe_codi_admin) && $depe_codi_admin != "0")
        $sql2.=" and d.depe_codi in ($depe_codi_admin)";
        
    $sql2.= " order by d.depe_nomb";
	
    $rsarea = $db->conn->Execute($sql2);
    
    if (!$rsarea) {
        $error = $db->conn->ErrorMsg();
        echo "<div class='advice'>Error al consultar áreas: $error <br> SQL: $sql2</div>";
        // Create a dummy object to prevent crash
        $rsarea = new ADORecordSet_empty();
    }
    }
...
<td class="listado5"    > 
<input align="center"  class="listado5" name="btn_asignar" id="btn_asignar" type="submit" value="    Asignar Área >>  " onclick="asignar_area(<?=$_SESSION['inst_codi']?>);"  /> <div id="cargando_div" style="display:none"><img src="../imagenes/progress_bar.gif" /></div> </td>
<td class="listado5"  >
<select name="Area" id="Area" size="20"  align="center" class="listado5">
    ...
    <table width="100%" class="borde_tab" align="center"  name="usr_permisos" id="usr_permisos">
     ...
     <Tr>      
      ...
      <TH > Área Asignada</TH>
      <TH >Acción
      <div id="cargando_div1" style="display:none"><img src="../../imagenes/progress_bar.gif" /></div> </TH>
     </Tr>
	...
	$sq = "select u.usua_codi, u.usua_nombre, u.usua_cedula, u.depe_nomb, u.inst_codi 
        ...
        // Fix: PHP 8 string comparison check (empty string vs 0)
        if (!empty($depe_codi_admin) && $depe_codi_admin != "0")
            $sq.=" and u.depe_codi in ($depe_codi_admin)";
    ...
    if (!$rsusuarea) {
        $error = $db->conn->ErrorMsg();
        echo "<div class='advice'>Error al consultar lista de usuarios: $error <br> SQL: $sq</div>";
        $rsusuarea = new ADORecordSet_empty();
    }
