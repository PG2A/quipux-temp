...
if (isset($pagina_anterior) and trim($ciu_email)!="") {
    ...
    $mail .= "</body></html>";
    enviarMail($mail, "Quipux: Actualización de datos.", $ciu_email, "$ciu_nombre $ciu_apellido", dirname(__DIR__, 2));
}
...
if (isset($_POST["ciu_codigo_eliminar"])) {
    if ($_POST["ciu_codigo_eliminar"]!=$ciu_codigo) {
        ...
        if (trim($rs->fields["CIU_EMAIL"])!="" && $rs->fields["CIU_EMAIL"]!=$ciu_email)
                 if ($grabar_ciu==1)
            enviarMail($mail, "Quipux: Actualización de datos.", $rs->fields["CIU_EMAIL"], "$ciu_nombre $ciu_apellido", dirname(__DIR__, 2));
    }
}
