...
if ($_SESSION["admin_institucion"]!=1) {
    die("Usted no tiene permisos para acceder a esta p&aacute;gina.");
}
...
if($autorizar == 0)
{
    ...
    $mail .= "</body></html>";    
    enviarMail($mail, "Quipux: Solicitud para generar y firmar documentos por parte de un ciudadano.", $ciu_mail, $ciu_nomb. " " . $ciu_apel, dirname(__DIR__, 2));
//    echo "enviarMail($mail, \"Quipux: Solicitud para generar y firmar documentos por parte de un ciudadano.\", $ciu_mail, $ciu_nomb. \" \" . $ciu_apel, dirname(__DIR__, 2));";
}
else
    {
    $ciu_nomb = limpiar_sql(trim($_POST["ciu_nombre"]));
