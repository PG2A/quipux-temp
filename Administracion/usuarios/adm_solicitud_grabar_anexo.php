...
$recordsolicitud["CIU_CODIGO"]       =  limpiar_sql($ciu_codigo);
    ...
    if($accion==0)
    {
        if($userfile == 'cedula')
            {
        unlink(dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_cedula.pdf');
        //$recordsolicitud["SOL_ESTADO"]       = 1;
        $recordsolicitud["SOL_CEDULA"]       = $_POST["hd_accion_cedula"];
        }
        else if($userfile == 'planilla')
            {
        unlink(dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_planilla.pdf');
        //$recordsolicitud["SOL_ESTADO"]       = 1;
        $recordsolicitud["SOL_PLANILLA"]       = $_POST["hd_accion_planilla"];
        }
        else if($userfile == 'acuerdo')
            {
        unlink(dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_acuerdo.pdf.p7m');
        //unlink(__DIR__."/bodega/ciudadanos/".$codigoCiuAr.'_acuerdo.pdf');
        //$recordsolicitud["SOL_ESTADO"]       = 1;
    ...
    if($_FILES['cedula']!= NULL)
    {
        ...
        $archivo_cedula = dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_cedula.pdf';
    ...
    if($_FILES['planilla']!= NULL)
    {
        ...
        $archivo_planilla = dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_planilla.pdf';
    ...
    if($_FILES['acuerdo']!= NULL)
    {
        ...
        $archivo_acuerdo = dirname(__DIR__, 2)."/bodega/ciudadanos/".$codigoCiuAr.'_acuerdo.pdf.p7m';
        if(is_file($solicitud_acuerdo))
            if ($tamanio>0){
             $bien3 = move_uploaded_file($solicitud_acuerdo,$archivo_acuerdo);
        $path_descarga = dirname(__DIR__, 2)."/bodega/ciudadanos/";

//        $archivo_acuerdo_pdf = __DIR__."/bodega/ciudadanos/".$codigoCiuAr.'_acuerdo.pdf';
...
//die();
       ...
       if (is_file($userfile))
         $nombre_archivo = "<table width='100%' align='center' border='1' class='t_bordeGris'><tr><td class='titulos3' width='13%'>$nombreTipoAnexo</td><td><a href='#' class='vinculos'>$nomb_arch</a>&nbsp;&nbsp;&nbsp;<img src='../imagenes/close_button.gif' width='20' height='18' onClick=\\\"anexo_borrar_archivo('$userfile');\\\" title='Eliminar Archivo' style='border: 0px solid gray;cursor:pointer;' alt='Eliminar Archivo'></td></tr></table>";
       else
           $nombre_archivo = "<table width='100%' align='center' border='1' class='t_bordeGris'><tr><td class='titulos2' width='11%'>* Acuerdo: </td><td class='listado3'><font color='black'>Error al subir archivo</font> <br> Intentar nuevamente &nbsp;<a href='../../Administracion/usuarios/adm_solicitud.php' class='vinculos'>aquí</a></td></tr></table>";
        ...
        $path_descarga = "../../archivo_descargar.php?path_arch=/ciudadanos/$nombre&nomb_arch=$nombre";
         ...
         $nombre_archivo .= "&nbsp;&nbsp;&nbsp;<img src='../../imagenes/close_button.gif' width='20' height='18' onClick=\\\"anexo_borrar_archivo('$userfile');\\\" title='Eliminar Archivo' style='border: 0px solid gray;cursor:pointer;' alt='Eliminar Archivo'>";
         $nombre_archivo .= "</td></tr></table>";
      }
