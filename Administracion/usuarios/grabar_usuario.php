...
include_once(dirname(__DIR__, 2)."/backup/permiso_autoriza_grabar_ajax.php");
...
// Emular register_globals para este script antiguo
foreach ($_REQUEST as $key => $value) {
    if ($key != 'PHPSESSID' && $key != 'krd') { // Evitar sobrescribir variables críticas
        $$key = $value;
    }
}
...
if(!isset($_REQUEST['usr_area_responsable']) || $_REQUEST['usr_area_responsable']==""){
...
if ($accion == 1) {
    ...
    // Intent with correct sequence name confirmed by user
    $usr_codigo = $db->nextId("usuarios_usua_codi_seq");
    
    // Explicit Fallback for ADOdb/Postgres issues
    if (!$usr_codigo) {
        $usr_codigo = $db->conn->GetOne("select nextval('usuarios_usua_codi_seq')");
    }
    
    // HARD DEBUG: Stop here and show the ID.
    // die("DEBUG: Generated User ID = " . var_export($usr_codigo, true));
...
$graba_usr = 1; // Default to true, disable if validation fails
...
if ($usr_estado==0) {
    ...
    if ($usr_cedula=='' || $usr_cedula==0) $graba_usr = 0;
    if($usr_tipo_id==1)
...
if ($usr_nombre=='' || $usr_nombre==0) $graba_usr = 0;
$record["USUA_NOMB"]            = $db->conn->qstr(limpiar_sql(trim($ciud->caracterEspecial($usr_nombre))));
if ($usr_apellido=='' || $usr_apellido==0) $graba_usr = 0;
...
if (isset($_FILES["firmaDigitalizada"]['name']) && trim($_FILES["firmaDigitalizada"]['name']) != "")
...
if (isset($_POST["usr_permiso_19"])) {
    $record["USUA_TIPO_CERTIFICADO"] = 0 + (int)$_POST["usr_tipo_certificado"];
...
if ($graba_usr==1){
    ...
    // Fix Case Sensitivity for Postgres: Lowercase table, PK, and columns
...
if($ok1){
    ...
    if (isset($_FILES["firmaDigitalizada"]['name']) && trim($_FILES["firmaDigitalizada"]['name']) != "") {
        ...
        $rutaFirma = __DIR__."/bodega/firmas/".$usr_codigo.'.'.($_POST["extarch"] ?? '');
...
if (!$rsper->EOF){
    $id_permiso12 = $rsper->fields['PERMISO_ADM'];
