...
$sql = "select substr(u.usua_cedula,1,10) as \"Cedula\"
  ...
  $estado = isset($_POST["estado"]) ? (int)$_POST["estado"] : 2;
  if ($estado != 2) 
      $sql .= " and u.usua_esta=" . $estado;

  $sql .= " order by 6,2 asc, 1 asc";
//echo $sql;
  echo "<!DOCTYPE html>" . html_head() . "<body>";

  $rs = $db->conn->Execute("select depe_nomb as nombre from dependencia where depe_codi = ".$area);
  if ($rs && !$rs->EOF)
...
<table class="borde_tab" width="100%">
  ...
  if (!$rsUsuarios) {
      echo "Error on query: " . $db->conn->ErrorMsg();
