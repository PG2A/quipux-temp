...
******************************************************************************************************/
    ...
    // Prepare all data in memory to avoid N+1 queries
    function cargar_datos_memoria() {
        if (trim($this->sql) == "") $this->set_sql($this->condicion);
        
        // Remove the **DATO** placeholder to get ALL relevant records
        $sql = str_replace("**DATO**", "", $this->sql);
        
        if ($this->display["debug"] ?? false) echo "<br>Loading memory cache: $sql";
        ...
        while (!$rs->EOF) {
            $padre = trim((string)$rs->fields["PADRE"]);
            if ($padre == "") $padre = "0"; // Handle null/empty parents
            
            $this->cache_data[$padre][] = $rs->fields;
            $rs->MoveNext();
        }
    }
    ...
    function listar_recursivo($codigo, $nivel) {
        if ($this->display["debug"] ?? false) echo "<br>listar_recursivo($codigo, $nivel)";
        ...
        // MODE 1: MEMORY CACHE
        if ($this->use_memory_cache) {
            // Memory Cache Logic
            if ($nivel == 0) {
                 // For level 0, we need to find rows where ID is in $codigo
                 // This effectively scans the whole cache to find roots if we don't index by ID.
                 // Given the typical hierarchy size, scanning is acceptable or we can optimize if needed.
                 // But wait, USUALLY level 0 is just "all" or specific roots.
                 // To match the query logic: "id_tabla in ($codigo)"
                 
                 $ids = explode(',', $codigo);
                 foreach ($this->cache_data as $p => $rows) {
                     foreach ($rows as $row) {
                         if (in_array(trim((string)$row['CODIGO']), $ids)) {
                             $records[] = $row;
                         }
                     }
                 }
            } else {
                 if (isset($this->cache_data[$codigo])) {
                     $records = $this->cache_data[$codigo];
                 }
            }
            
        } else {
            // MODE 2: STANDARD DB QUERY
            if ($nivel==0) {
            ...
            if ($this->display["debug"] ?? false) echo "<br>$sql";
            $rs = $this->db->query($sql);
        ...
        if ($nivel!=0) $this->variables_jsc .= "var REC_$padre_js = new Array();";
        
        if (empty($records)) return;

        foreach ($records as $row) {
            $codigo = $row["CODIGO"]; // Update current code
            
            // Prevent infinite loop if parent == child
            if ($nivel > 0 && $codigo == $padre_js) continue;

            if ($nivel!=0) $this->variables_jsc .= "REC_$padre_js.push('$codigo'); ";
            ...
            for ($i=0 ; $i<$num_campo ; ++$i) {
                $campo_val = $row[strtoupper($this->campo[$i]["alias"])];
                if ($this->campo[$i]["tipo"] == "L") {
                    $campo_val = '<a href="javascript:;" onclick='.$row[strtoupper('f'.$this->campo[$i]['alias'])].' '. $this->display["link"].' > '.$campo_val.'</a>';
                }
                $this->respuesta .= "<td>$espacios $campo_val</td>";
                $espacios = "";
            }
            $this->respuesta .= "</tr>";
...
include __DIR__."/recursivas/funciones_recursivas.php";
