...
{
	$sql = "SELECT COUNT(*) FROM RADICADO 
			WHERE SGD_TMA_CODIGO IN (SELECT SGD_TMA_CODIGO FROM SGD_TMA_TEMAS WHERE SGD_TMA_DESCRIP='$dato')";
	...
	{
		$this->cnn->BeginTrans();
		$ok = $this->cnn->Execute('DELETE FROM SGD_DCAU_CAUSAL WHERE SGD_CAU_CODIGO='.$dato);
		if ($ok)
