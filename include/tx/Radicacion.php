...
/*
--esto se agrego poruqe no funcionaba la generacion del numero en base a la secuencia
try{
$query = "SELECT NEXTVAL('sec_radi_nume_radi')";
$dbconn = pg_connect("host=172.16.0.35 dbname=migracion user=postgres password=@ustr0g@$")
    or die('No se ha podido conectar: ' . pg_last_error());
	
$result = pg_query($query) or die("<hr><b><font color=red><center>Lo sentimos, existieron errores al generar la secuencia.</center></font></b><hr>" . pg_last_error());
...
*/		
        // Use a direct SAVEPOINT-guarded NEXTVAL to prevent transaction abort on failure.
        // ADODB's GenID() tries CREATE SEQUENCE on failure, which further aborts the transaction.
        $secNew = 0;
        $seqErr = '';
        $this->db->conn->Execute("SAVEPOINT sp_nextval");
        $rs_seq = $this->db->conn->Execute("SELECT nextval('sec_radi_nume_radi') AS next_id");
        if ($rs_seq && !$rs_seq->EOF) {
            $secNew = (int)($rs_seq->fields['NEXT_ID'] ?? $rs_seq->fields['next_id'] ?? 0);
            $this->db->conn->Execute("RELEASE SAVEPOINT sp_nextval");
        } else {
            $seqErr = $this->db->conn->ErrorMsg();
            $this->db->conn->Execute("ROLLBACK TO SAVEPOINT sp_nextval");
        }
        if ($secNew == 0) {
            include_once(dirname(__DIR__,2).'/funciones_interfaz.php');
            die(html_error("Lo sentimos, existieron errores al generar la secuencia.<br>Error: ".htmlspecialchars($seqErr)."<br><small>Secuencia: ".$secNew." Transacci&oacute;n: ".htmlspecialchars((string)$this->transaccion)."</small>"));
        }
        ...
        if ($this->flagRadiTexto==1 and trim((string)$this->radiNumeText)=="") {
            $noRadText = $this->GenerarTextRadicado($formatoRadicado, $this->radiTipo, "N");
        } else {
            if (trim((string)$this->radiNumeText)=="")
                $noRadText = $this->GenerarTextRadicado($formatoRadicado, $this->radiTipo, "T");
            else
                $noRadText = trim((string)$this->radiNumeText);
        }
        //echo die("<hr><b><font color=red><center>NUMRAD $newRadicado <br>TEXTRAD $noRadText</center></font></b><hr>");
        if ($this->flagRadiTexto==1 and trim((string)$this->radiNumeText)!="") $this->radiTextTemp = $this->radiNumeText; //Si es un documento hijo externo
        ...
        if(!$this->radiSeguridad) $this->radiSeguridad = "1";
        ...
        if (trim((string)$this->radi_tipo_impresion) != "999")
        ...
        $recordR["RADI_DESC_ANEX"]	= $this->db->conn->qstr(substr((string)$this->radiDescAnex,0,100));
        $recordR["COD_CODI"] = (int)$this->cod_codi;
        $recordR["CAT_CODI"] = (int)$this->cat_codi;
        if ($this->radiTexto!="0") 	$recordR["RADI_TEXTO"] = $this->radiTexto;
        if(trim((string)$this->radi_imagen)!="") $recordR["RADI_IMAGEN"] = $this->db->conn->qstr($this->radi_imagen);

		
        $insertSQL = $this->db->conn->Replace("RADICADO", $recordR, "", false, false);
		
        if($insertSQL) {
            if ($this->radiTexto=="0") $insertSQL=$this->updateTextoRadicado($newRadicado);
        }
        if(!$insertSQL) {
            echo "<hr><b><font color=red>Error no se inserto el registro<br>Error: ".$this->db->conn->ErrorMsg()."</font></b><hr>";
            $newRadicado = null;
