...
// Defensive defaults to avoid undefined variable notices when included
$ruta_raiz = $ruta_raiz ?? '';
$whereFiltro = $whereFiltro ?? '';
$carpeta = $carpeta ?? 1;
$orderNo = $orderNo ?? '';
$orderTipo = $orderTipo ?? 'desc';
$version_light = $version_light ?? false;
$txt_fecha_desde = $txt_fecha_desde ?? '';
$txt_fecha_hasta = $txt_fecha_hasta ?? '';
$descZonaHoraria = $descZonaHoraria ?? '';
$descRadicado = $descRadicado ?? '';
$txt_start_date = $txt_start_date ?? '';
$txt_end_date = $txt_end_date ?? '';

/**
 * Global annotations for static analysis
 * @global \ADOConnection $db
 * @global int $carpeta
 * @global string $whereFiltro
 */



function get_inbox_data($inbox, $txt_start_date, $txt_end_date, $orderType, $orderNro, $whereFilter = '') {
    global $db, $CFG, $descZonaHoraria;

    if ($db->driver == 'postgres') {
        $sql_user_data = "(SELECT u.usua_codi AS usua_codi, 
                                concat_ws(' ', u.usua_nomb, u.usua_apellido) AS usua_nombre, 
                                u.depe_codi AS depe_codi, 
                                u.inst_codi AS inst_codi
                            FROM usuarios u
                            UNION ALL 
                            SELECT c.ciu_codigo AS ciu_codigo, 
                            concat_ws(' ', c.ciu_nombre, c.ciu_apellido),
                            0,
                            0
                            FROM ciudadano c) AS ";

        $sql_user_data_print = "(SELECT usua_codi,
                                    COALESCE(usua_nomb, '') || ' ' || COALESCE(usua_apellido, '') AS usua_nombre,
                                    depe_codi,
                                    inst_codi
                                FROM usuarios
                                WHERE depe_codi = ".(int)$_SESSION["depe_codi"].") AS ";

        $sql_document_date = "(CASE WHEN radi_nume_temp::text like '%0'
                            THEN radi_fech_ofic
                            ELSE radi_fech_radi
                            END)";

        $where_document_date = "";
        if($CFG->version_light && !empty($txt_start_date) && !empty($txt_end_date)) {
            $where_document_date = " AND ($sql_document_date::date BETWEEN '$txt_start_date' AND '$txt_end_date')";
        }

        $sql_user_info = " - USR ".(int)$_SESSION["usua_codi"]." - ".date("Y-m-d H:i:s")."<br>";

        $html_signature = '<div><a href="#" class="Ntooltip"><img src="../../imagenes/key-yellow.png" alt="Firmado digitalmente"><span>Firmado digitalmente</span></div>';
        $html_urgent = '<div><a href="#" class="Ntooltip"><img src="../../imagenes/urgente.png" width="16" height="16" alt="Urgente"><span>Urgente</span></div>';

        $sql = '';
        switch ($inbox) {
            case 1:
                if (isset($CFG->replicacion) && $CFG->replicacion && !($CFG->config_db_replica_cuerpo_paginador === "")) {
                    $db = new ConnectionHandler(dirname(__DIR__, 2), $CFG->config_db_replica_cuerpo_paginador);
                }

                $usua_codi = (int)$_SESSION["usua_codi"];
                $inst_codi = (int)$_SESSION["inst_codi"];

                $sql = "SELECT CAST(radi_nume_radi AS varchar) as CHK_CHKANULAR,
                            CASE WHEN UPPER(cat_descr)='URGENTE' THEN 'urgente_html' ELSE 'img' AS SCR_,
                            'mostrar_documento(\"'|| radi_nume_radi ||'\",\"'||radi_nume_text||'\",\"'||$inbox||'\")' AS HID_POPUP,
                    ...
                    FROM (SELECT b.radi_nume_radi,
                                ...
                                b.radi_tipo, 
                                b.radi_leido
                            FROM (SELECT *, $sql_document_date AS fecha_documento 
                                    FROM radicado b 
                                    WHERE radi_usua_actu = $usua_codi
                                        AND esta_codi = 1
                                        AND radi_inst_actu = $inst_codi  
                                        $whereFilter 
                                        $where_document_date
                                        AND radi_nume_radi NOT IN (SELECT radi_nume_radi FROM tarea WHERE estado = 1 AND usua_codi_ori = $usua_codi)) as b
                                    LEFT OUTER JOIN categoria cat ON coalesce(b.cat_codi, 0) = cat.cat_codi
                                    ORDER BY ".($orderNro+1)." $orderType, radi_nume_radi $orderType)
                            AS a ORDER BY ".($orderNro+1)." $orderType, radi_nume_radi $orderType";
                break;
...
switch($db->driver) {
    ...
    $popup='<img src="'.$ruta_raiz.'/iconos/popup.png" border=0>';
    
    switch ($carpeta) {

        case 1:  // En elaboración
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_elaboracion) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_elaboracion);
            if ($orderNo=='') $orderNo=7;
            $isql = "select -- En elaboracion $info_usuario_query
                    CAST(radi_nume_radi AS varchar) as \"CHK_CHKANULAR\"
        ...
        case 2:  // Recibidos
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_recibidos) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_recibidos);
        ...
        case 84:  // Ciudadanos Firma - Eliminados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador != $config_db_replica_bandeja_eliminados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_eliminados);
        ...
        case 7:  // No enviados, sin firma digital
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_no_enviados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_no_enviados);
        ...
        case 8:  // Enviados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_enviados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_enviados);
            if ($orderNo=='') $orderNo=8;
        ...
        case 10: //Archivados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_archivados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_archivados);
            if ($orderNo=='') $orderNo=8;
        ...
        case 12:  // Bandeja Reasignados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_reasignados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_reasignados);
            if ($orderNo=='') $orderNo=6;
        ...
        case 13: // Bandeja Informados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_informados) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_informados);
            if ($orderNo=='') $orderNo=6;
        ...
        case 80:  // Enviados Ciudadano
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_enviados_ciudadanos) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_enviados_ciudadanos);
            if ($orderNo=='') $orderNo=6;

            if ($version_light) $where_fecha_documento = " and (radi_fech_radi::date between '$txt_fecha_desde' and '$txt_fecha_hasta')";

            $isql = "select -- Enviados Ciudadanos $info_usuario_query
                      radi_nume_radi as \"CHK_CHKANULAR\",'<img src=\"../iconos/popup.png\" border=0>' as \"SCR_ \"
                    , 'mostrar_documento(\"'||radi_nume_radi||'\",\"'||radi_nume_text||'\",\"'||$carpeta||'\")' as \"HID_POPUP\"
        ...
        case 81:  // Recibidos Ciudadano
            ...
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_recibidos_ciudadanos) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_recibidos_ciudadanos);
            if ($orderNo=='') $orderNo=6;

            if ($version_light) $where_fecha_documento = " and (radi_fech_radi::date between '$txt_fecha_desde' and '$txt_fecha_hasta')";
            
            $isql = "select  -- Recibidos Ciudadanos $info_usuario_query
                      radi_nume_radi as \"CHK_CHKANULAR\",'$popup' as \"SCR_ \"
                    , 'mostrar_documento(\"'||radi_nume_radi||'\",\"'||radi_nume_text||'\",\"'||$carpeta||'\")' as \"HID_POPUP\"
        ...
        case 90: //Pendientes recibidos de ciudadanos
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_pendientes_ciudadanos) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_pendientes_ciudadanos);
            if ($orderNo=='') $orderNo=6;
        ...
        case 99:  // Documentos por imprimir, envío manual
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_por_imprimir) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_por_imprimir);
            if ($orderNo=='') $orderNo=9;
        ...
        case 82:  // Ciudadanos Firma - En elaboración
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_elaboracion_ciudadanos_firma) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_elaboracion_ciudadanos_firma);
            if ($orderNo=='') $orderNo=5;
        ...
        case 83:  // Recibidos
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_recibidos_ciudadanos_firma) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_recibidos_ciudadanos_firma);
            if ($orderNo=='') $orderNo=6;
        ...
        case 85:  // No enviados, sin firma digital
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_no_enviados_ciudadanos_firma) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_no_enviados_ciudadanos_firma);
            if ($orderNo=='') $orderNo=5;
        ...
        case 86:  // Enviados
            if (isset ($replicacion) && $replicacion && $config_db_replica_cuerpo_paginador!=$config_db_replica_bandeja_enviados_ciudadanos_firma) $db = new ConnectionHandler(__DIR__,$config_db_replica_bandeja_enviados_ciudadanos_firma);
            if ($orderNo=='') $orderNo=7;
