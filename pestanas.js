...
function mostrar_botones(estado, carpeta, tiporad) {
    ...
    if ('<?=(!empty($verrad) && $pagactual==1)?1:0?>' == '1') document.getElementById('btn_imgRegresar').style.display = '';
    ...
    if (estado == 100) {  //Ciudadanos   
        ...
        if (carpeta == 12) {
            document.getElementById('btn_informar').style.display = '';
            if ('<?=(isset($_GET["tipo_ventana"]) ? $_GET["tipo_ventana"] : "")?>' != 'popup') document.getElementById('btn_recuperar').style.display = '';
            if ('<?=$pagactual?>' == '1') document.getElementById('btn_copiar_documento').style.display = '';
        }
