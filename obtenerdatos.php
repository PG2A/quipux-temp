...
**/
    function ObtenerDatosOpcImpresion($radicado,$db){
        if (trim((string)$radicado)=="") return array();
        $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $isql="select * from opciones_impresion where radi_nume_radi=".trim((string)$radicado)." order by opc_imp_codi desc" ;
        $rs=$db->query($isql);

        if (!$rs || $rs->EOF) return array();

        $vector["OPC_IMP_CODI"] = $rs->fields["OPC_IMP_CODI"] ?? null;
        $vector["RADI_NUME_RADI"] = $rs->fields["RADI_NUME_RADI"] ?? null;
        $vector["OCULTAR_NUME_RADI"] = $rs->fields["OPC_IMP_OCULTAR_NUME_RADI"] ?? null;
        $vector["MOSTRAR_PARA"] = $rs->fields["OPC_IMP_MOSTRAR_PARA"] ?? null;
        $vector["JUSTIFICAR_FIRMA"] = $rs->fields["OPC_IMP_JUSTIFICAR_FIRMA"] ?? null;
        $vector["TITULO_NATURAL"] = $rs->fields["OPC_IMP_TITULO_NATURAL"] ?? null;
        $vector["EXT_INSTITUCION"] = $rs->fields["OPC_IMP_EXT_INSTITUCION"] ?? null;
        $vector["DESTINO_DESTINATARIO"] = $rs->fields["OPC_IMP_DESTINO_DESTINATARIO"] ?? null;
        $vector["FRASE_REMITENTE"]= $rs->fields["OPC_IMP_FRASE_REMITENTE"] ?? null;
        $vector["FRASE_DESPEDIDA"]= $rs->fields["OPC_IMP_DESPEDIDA"] ?? null;
        $vector["DIRECCION"]= $rs->fields["OPC_IMP_DIRECCION"] ?? null;
        $vector["CIUDAD"]= $rs->fields["OPC_IMP_CIUDAD"] ?? null;
        $vector["TELEFONO"]= $rs->fields["OPC_IMP_TELEFONO"] ?? null;
        $vector["FIRMANTES"] = $rs->fields["OPC_IMP_FIRMANTES"] ?? null;
        $vector["OCULTA_FRASE_REMITENTE"] = $rs->fields["OPC_IMP_OCULTAR_FRASE_REM"] ?? null;
        $vector["CARGO_CABECERA"] = $rs->fields["OPC_IMP_CARGO_CABECERA"] ?? null;
        $vector["TEXTO_SOBRE"] = $rs->fields["OPC_IMP_TEXTO_SOBRE"] ?? null;
        $vector["OPC_IMP_TIPO_NOTA"] = $rs->fields["OPC_IMP_TIPO_NOTA"] ?? null;
        $vector["OPC_JUSTIFICAR_FECHA"] = $rs->fields["OPC_IMP_JUSTIFICAR_FECHA"] ?? null;
        $vector["OCULTAR_ASUNTO"] = $rs->fields["OPC_IMP_OCULTAR_ASUNTO"] ?? null;
        $vector["LETRA_ITALICA"] = $rs->fields["OPC_IMP_LETRA_ITALICA"] ?? null;
        $vector["OCULTAR_ATENTAMENTE"] = $rs->fields["OPC_IMP_OCULTAR_ATENTAMENTE"] ?? null;
        $vector["OCULTAR_ANEXO"] = $rs->fields["OPC_IMP_OCULTAR_ANEXO"] ?? null;
        $vector["OCULTAR_REFERENCIA"] = $rs->fields["OPC_IMP_OCULTAR_REFERENCIA"] ?? null;
        $vector["OCULTAR_SUMILLAS"] = $rs->fields["OPC_IMP_OCULTAR_SUMILLAS"] ?? null;
        $vector["CIUDAD_DADO_EN"] = $rs->fields["OPC_IMP_CIUDAD_DADO_EN"] ?? null;
...
function ObtenerDatosOpcImpresionSobre($radiNumeRadi,$usuaCodi,$db){
        if (trim((string)$radiNumeRadi)=="" and trim((string)$usuaCodi)=="") return array();
        $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        $isql="select *, c.nombre as opc_imp_sob_ciudad
                ...
                where radi_nume_radi=".trim((string)$radiNumeRadi)." and usua_codi = ".trim((string)$usuaCodi) ;
...
**/
    ...
    {//Obtiene datos del radicado en función del texto
        ...
        $isqlTEXT = "select r.*, e.esta_desc, tr.trad_descr,
                     ...
                     where r.RADI_NUME_RADI=".trim((string)$radicado);
        $rs=$db->query($isqlTEXT);
...
//        $vector["radi_text_temp"] = $rs->fields["RADI_TEXT_TEMP"];
        ...
        $vector["radi_resumen"] = stripcslashes((string)$rs->fields["RADI_RESUMEN"]);
        $vector["radi_codi_texto"] = $rs->fields["RADI_TEXTO"];
        $vector["radi_asunto"] = stripcslashes((string)$rs->fields["RADI_ASUNTO"]);
        ...
        /* */
        //var_dump($rs);
        //echo "<hr> $vector <br> TEXTRAD=".$vector["cca"];
		$_SESSION["array_radicado"]=$vector;
		//var_dump($_SESSION["array_radicado"]);
        return $vector;
    }
...
**/
    ...
    {//Obtiene datos del usuario en función del texto
        if (trim((string)$codigo)=="") return array();
        $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        ...
        // Guard: query failed or returned no rows (e.g. empty/invalid user code)
        if (!$rs || $rs->EOF) return array();

        $vector["usua_codi"] =trim((string)$rs->fields["USUA_CODI"]);
        $vector["cedula"] =trim((string)$rs->fields["USUA_CEDULA"]);
        $vector["login"] =trim((string)$rs->fields["USUA_LOGIN"]);
        $vector["nombre"] = $rs->fields["USUA_NOMBRE"];
        $vector["usua_nombre"] =$rs->fields["USUA_NOMB"];
        $vector["usua_apellido"] =$rs->fields["USUA_APELLIDO"];
        $vector["inst_codi"] =trim((string)$rs->fields["INST_CODI"]);
        $vector["institucion"] =$rs->fields["INST_NOMBRE"];
        $vector["inst_estado"] =trim((string)$rs->fields["INST_ESTADO"]);
        $vector["usua_estado"] =trim((string)$rs->fields["USUA_ESTA"]);
    ...
    {//Verifica si un usuario tiene un determinado permiso
        if (trim((string)$codigo)=="" or trim((string)$permiso)=="") return 0;
...
**/
    ...
    { //Obtiene datos de la area en función del texto
        if (trim((string)$dependencia)=="") return array('nombre'=>'','sigla'=>'','ciudad'=>'');
        $db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
        ...
        if (!$rs || $rs->EOF) return array('nombre'=>'','sigla'=>'','ciudad'=>'');
 ...
 **/
    function ObtenerDatosLista($lst_codigo,$db){
        if (trim((string)$lst_codigo)=="") return array();
 ...
 **/
    function ObtenerUltimaFecha($radi_nume_radi, $transaccion ,$db){
        if (trim((string)$radi_nume_radi)=="" or trim((string)$transaccion)=="") return "";
 ...
 **/
    function ObtenerCiudadUsua($tabla, $condicion="", $db=null){
        if (trim((string)$tabla)=="") return array();
        $sql = "select id, nombre from $tabla";
        if(trim((string)$condicion)!='')
            $sql.= " where $condicion";
        //echo $sql;
        $rs=$db->query($sql);
 ...
 */
    function ObtenerCategoria($cat_codi, $db){
        if (trim((string)$cat_codi)=="") return array();
        if($cat_codi=="")
            $cat_codi = 0;
 ...
 */
    function ObtenerTipificacion($inst_codi, $cod_codi, $db){
        if (trim((string)$inst_codi)=="") return array();
        if($cod_codi=="")
            $cod_codi = 0;
 ...
 */
    function ObtenerJefeArea($inst_codi, $depe_codi, $cargo_tipo, $db){
        if (trim((string)$inst_codi)=="" or trim((string)$depe_codi)=="") return array();
    ...
    function ObtenerObservacionCiudadano($usua_codi, $datosArray, $db){
        $datosArray = array_change_key_case($datosArray, CASE_UPPER);
    ...
    function ObtenerListaUsuariosDocumento ($db, $radi_nume, $tipo="R") {
        $datos = array();
        if (strlen(trim((string)($radi_nume ?? ''))) != 20) return array();
        ...
        if ($estado==1 or $estado==7) { // Si el documento está en elaboración
            ...
            elseif ($tipo=="D") {
                ...
                foreach (explode("-", (string)($rs->fields["RADI_LISTA_DEST"] ?? '')) as $tmp) {
                    if ((int)$tmp > 0) {
            ...
            $lista_usr = trim(str_replace ("-", "", str_replace ("--", ",", (string)($lista_usr ?? ''))));
            if ($lista_usr=="") return array();
  ...
  if ($usr_codigo!=''){
    if ($perfil==1){//perfil administrador
        ...
        $sql ="select * from usuario_dependencia u    
           ...
           $depeCodiTmp = '';
           if ($rsDepePadre && !$rsDepePadre->EOF) {
               $depeCodiTmp = $rsDepePadre->fields['DEPE_CODI_TMP'] ?? '';
           }
          
           $depeOrdenTmp = str_replace(',0,', ',', (string)$depeCodiTmp);
            ...
            foreach ($radicadosSel as $radi_nume) {//for
              ...
              if ($radi_nume!=''){
                ...
                if ($radi_nume_padre!=''){//if radi seleccionado
                    ...
                    //nombres de usuarios
                    $usNombreUsuarios = ''; // initialize before loop
...
function ObtenerRutaMembrete($depe_codi, $trad_codigo, $inst_codi, $db)
{
    $raiz = __DIR__;
    $depe = (int)$depe_codi;
    $trad = (int)$trad_codigo;
    $inst = (int)$inst_codi;
    $existe_tabla = $db->conn->GetOne("select 1 from information_schema.tables where table_schema='public' and table_name='membrete'");
    if ($existe_tabla) {
        $rs = $db->conn->Execute("select m.memb_codi, m.memb_archivo, case when a.depe_codi is not null then 'area' else 'tipo' end as origen
                                  from membrete_asignacion a join membrete m on m.memb_codi=a.memb_codi
                                  where a.masi_estado=1 and m.memb_estado=1 and m.inst_codi=$inst and a.masi_uso='documento'
                                    and (a.depe_codi=$depe or a.depe_codi is null)
                                    and (a.trad_codigo=$trad or a.trad_codigo is null)
                                    and not (a.depe_codi is null and a.trad_codigo is null)
                                  order by case when a.depe_codi is not null and a.trad_codigo is not null then 1
                                                when a.depe_codi is not null then 2
                                                else 3 end
                                  limit 1");
        if ($rs && !$rs->EOF) {
            $ruta = "$raiz/bodega/plantillas/membretes/" . basename((string)$rs->fields["MEMB_ARCHIVO"]);
            if (is_file($ruta)) return array("ruta" => $ruta, "memb_codi" => (int)$rs->fields["MEMB_CODI"], "origen" => (string)$rs->fields["ORIGEN"]);
        }
        $rs = $db->conn->Execute("select memb_codi, memb_archivo from membrete where inst_codi=$inst and memb_defecto=1 and memb_estado=1 limit 1");
        if ($rs && !$rs->EOF) {
            $ruta = "$raiz/bodega/plantillas/membretes/" . basename((string)$rs->fields["MEMB_ARCHIVO"]);
            if (is_file($ruta)) return array("ruta" => $ruta, "memb_codi" => (int)$rs->fields["MEMB_CODI"], "origen" => "defecto");
        }
    }
    $area = ObtenerDatosDependencia($depe, $db);
    $ruta = "$raiz/bodega/plantillas/" . (int)($area["plantilla"] ?? 0) . ".pdf";
    if (is_file($ruta)) return array("ruta" => $ruta, "memb_codi" => 0, "origen" => "area_legado");
    return array("ruta" => "", "memb_codi" => 0, "origen" => "ninguno");
}
?>
