...
$tabla .= "<tr $estilo_titulo><td colspan='2'><font size='1'><b>Reporte de Usuarios</b></font></td></tr>";
$tabla .= "<tr><td $estilo_tr1><font size='1'>Area:</font></td><td><font size='1'>".$area_nombre."</font></td></tr>";
...
$tabla .= "<tr $estilo_titulo>";
        ...
        $tabla .= "<td><font size='1'><b>Identificación</b></font></td>";
        $tabla .= "<td><font size='1'><b>Título</b></font></td>";
...
$sql = "select usua_cedula as identificacion, usua_titulo as titulo, usua_nombre as nombre, usua_cargo as cargo, usua_email as email, depe_nomb as dependencia  
		from usuario
	    where depe_codi=7 and usua_esta=1 and  inst_codi=3 
		order by usua_nombre";
...
$html = preg_replace(':<a.*?>:is', '', $html);
    ...
    require_once(__DIR__.'/interconexion/generar_pdf.php');
        require_once(__DIR__.'/obtenerdatos.php');
        ...
        file_put_contents(__DIR__."/bodega$path_archivo", $pdf);
        $path_descarga = __DIR__."/archivo_descargar.php?path_arch=$path_archivo&nomb_arch=reporte.pdf";
}
